...
def pre_process():
    ...
    if settings:
        ...
        if settings.floor3_mode_active and not getattr(settings, 'floor3_paused', False) and settings.vote_end_time and now >= settings.vote_end_time:
            if getattr(settings, 'floor3_results_active', False):
...
def home():
    ...
    if settings and getattr(settings, 'floor1_mode_active', False) and user and user.status == 'active':
        ...
        # غرفة البصيرة - تظهر نقاط اللاعبين بدون أسماء
        ...
        # مرصد الأبراج - يظهر اللاعبين في الغرف المجاورة بأسمائهم
    ...
    if request.method == 'POST':
        ...
        if act == 'toggle_gates':
            ...
            if getattr(settings, 'floor1_mode_active', False):
                ...
                User.objects.update(set__f1_tasks=[], set__current_room=None, set__group_id=None, set__is_cursed=False, set__gems_collected=0, set__used_vent=False, set__emergency_used=False)

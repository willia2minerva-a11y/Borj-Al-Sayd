...
def process_f1_meeting_end(settings):
    groups = User.objects(status__in=['active', 'inactive'], group_id__gt=0).distinct('group_id')
    for gid in groups:
...
def register():
    if request.method == 'POST':
        ...
        flash('تم التسجيل! حسابك قيد المراجعة للعب.', 'success'); return redirect(url_for('login'))
    ...
    if target_user:
        try:
            ...
            if action == 'edit_name': target_user.update(set__username=val)
                ...
                hp = int(val or 0); target_user.update(set__health=hp, set__status='eliminated' if hp <= 0 else 'active')

...
# 🚨 إعدادات قاعدة البيانات المحسنة لمنع تجمد السيرفر (Timeout)
    ...
    if getattr(settings, 'maintenance_mode', False):
        ...
        if m_until and datetime.utcnow() > m_until:
            ...
            if 'all' in m_pages or request.endpoint in m_pages:
                return render_template('locked.html', message='جاري ترميم ونقش هذه الصفحة ⏳')
...
def register():
    if request.method == 'POST':
        ...
        # 🚨 الجميع يبدأ من منطقة "البوابات" وبرتبة "مستكشف"
        User(
            hunter_id=new_id, 
        ...
        if has_shield:
            ...
            if target.health <= 0: 
                ...
                if has_totem:
                    ...
                    # 🚨 نظام النقل الآلي للطابق الثاني عند انتهاء الحرب
                    if getattr(settings, 'war_mode', False) and settings.dead_count >= getattr(settings, 'war_kill_target', 15):
                        settings.war_mode = False
                        log_action(f"🛑 اكتفت المتاهة من الدماء. الناجون يصعدون للطابق 2!", "system", is_epic=True)
                        User.objects(status='active', role='hunter', hunter_id__ne=1000).update(set__zone='الطابق 2')
...
def puzzles():
    ...
    if request.method == 'POST':
        ...
        # التحقق من الإجابة (سواء كان لغز نصي، أو من الجافاسكريبت للضغط والترتيب)
                ...
                for uid in request.form.getlist('selected_users'):
                    ...
                    if u and getattr(u, 'hunter_id', 0) != 1000:
                        if bt == 'hard_delete': 
                            ...
                            u.save()
                        elif bt == 'move_zone': # 🚨 النقل الآلي واليدوي

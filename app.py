...
# 🗺️ خريطة المتاهة المعمارية للطابق الأول
F1_MAP = {
...
def pre_process():
    ...
    try: 
        ...
        return Response(f"<div style='direction:rtl; background:#0a0a0a; color:#e74c3c; padding:30px; text-align:center;'><h1>🚨 مشكلة اتصال بقاعدة البيانات</h1></div>", 503)
...
# 🚀 تم تفعيل الكاش لتسريع الموقع ومنع انهيار الرندر
...
def profile():
    ...
    return render_template('profile.html', user=user, my_items=my_items, my_seals=my_seals)
...
def admin_panel():
    ...
    if request.method == 'POST':
        ...
        if act == 'toggle_gates':
            ...
            if getattr(settings, 'floor1_mode_active', False):
                ...
                User.objects.update(set__f1_tasks=[], set__current_room='قاعة العروش', set__group_id=0, set__is_cursed=False, set__gems_collected=0, set__used_vent=False, set__emergency_used=False)
                flash('تم إيقاف الطابق الأول وتصفير اللاعبين.', 'success')
            else:
            ...
            if gid and str(gid) in getattr(settings, 'f1_active_meetings', {}):
                meetings = settings.f1_active_meetings
            ...
            if getattr(settings, 'floor3_mode_active', False):
                ...
                new_end = datetime.utcnow() + timedelta(seconds=getattr(settings, 'floor3_time_left', 0))
            ...
            pt = request.form.get('puzzle_type', 'none')
            ans = request.form.get('puzzle_answer', '') if pt != 'none' else ''
            News(title=request.form.get('title'), content=request.form.get('content'), category='puzzle' if pt != 'none' else 'news', puzzle_type=pt, puzzle_answer=ans, reward_points=int(request.form.get('reward_points') or 0), max_winners=int(request.form.get('max_winners') or 1), author=g.user.username, status='approved').save()
            flash('تم إصدار المرسوم!', 'success')
            ...
            try: SpellConfig.objects(id=request.form.get('spell_id')).delete()
            except: pass
            flash('تم إبطال التعويذة!', 'success')
                
        elif act == 'setup_maintenance':
            dur = int(request.form.get('m_duration') or 0)
            if dur > 0:
                GlobalSettings.objects(setting_name='main_config').update_one(set__maintenance_mode=True, set__maintenance_until=datetime.utcnow() + timedelta(minutes=dur), set__maintenance_pages=request.form.getlist('m_pages'))
                flash('تم تفعيل وضع الصيانة!', 'success')
            else:
                GlobalSettings.objects(setting_name='main_config').update_one(set__maintenance_mode=False, set__maintenance_pages=[])
                flash('تم إنهاء وضع الصيانة!', 'success')
    ...
    all_news = News.objects(category__in=['news', 'puzzle', 'declaration']).order_by('-created_at')[:50]
    spells = SpellConfig.objects().order_by('-created_at')
    
    all_hunters = User.objects(hunter_id__ne=1000)
    admin_stats = {
        'total': all_hunters.count(),
        'alive': all_hunters.filter(status='active').count(),
        'dead': all_hunters.filter(status__in=['eliminated', 'dead_body']).count(),
        'points': all_hunters.sum('points') or 0
    }
    
    return render_template('admin.html', users=users_query.order_by('-last_active')[:100], settings=settings, search_query=search_query, hidden_traps=hidden_traps, all_news=all_news, spells=spells, stats=admin_stats)
...
def update_settings():
    ...
    try:
        if action == 'change_avatar':
            ...
            new_name = request.form.get('new_name')
            if getattr(user, 'last_name_change', None) and (now - user.last_name_change).days < 15: flash('تغيير الاسم متاح مرة كل 15 يوم!', 'error')
            elif User.objects(username=new_name).first(): flash('الاسم مستخدم مسبقاً!', 'error')
            else: user.update(set__username=new_name, set__last_name_change=now); flash('تم تغيير الاسم!', 'success')
        elif action == 'change_password':
            if check_password_hash(user.password_hash, request.form.get('old_password', '')) and request.form.get('new_password') == request.form.get('confirm_password'):
                user.update(set__password_hash=generate_password_hash(request.form.get('new_password')), set__last_password_change=now); flash('تم تغيير الختم السري!', 'success')
    except: flash('حدث خطأ!', 'error')
    return redirect(url_for('profile'))
...
def add_friend():
    ...
    if getattr(target, 'role', '') in ['ghost', 'cursed_ghost']:
        ...
        else: flash('اختفى الشبح أو نفدت مكافآته.', 'info')
    elif target.hunter_id not in getattr(user, 'friends', []) and user.hunter_id not in getattr(target, 'friend_requests', []): 
        target.update(push__friend_requests=user.hunter_id); flash('أُرسل طلب التحالف.', 'success')
...
def cancel_request():
    ...
    if target and user.hunter_id in getattr(target, 'friend_requests', []): target.update(pull__friend_requests=user.hunter_id); flash('تم سحب الطلب.', 'info')
    return redirect(request.referrer or url_for('home'))
...
def accept_friend(friend_id):
    ...
    if friend and friend.status in ['active', 'inactive'] and friend_id in getattr(user, 'friend_requests', []): 
        user.update(pull__friend_requests=friend_id, push__friends=friend_id); friend.update(push__friends=user.hunter_id); check_achievements(user)
    return redirect(request.referrer or url_for('home'))
...
def use_item(target_id):
    ...
    if item_type in ['weapon', 'steal', 'spy']:
        ...
        if cooldown_mins > 0 and attacker.last_action_time and (now - attacker.last_action_time).total_seconds() < (cooldown_mins * 60):
            flash(f'الرجاء الانتظار {int((cooldown_mins*60) - (now - attacker.last_action_time).total_seconds())} ثانية.', 'error'); return redirect(request.referrer or url_for('home'))
    if getattr(settings, 'final_battle_mode', False) and target.hunter_id != 1000 and item_type in ['weapon', 'steal', 'spy']: flash('الإمبراطور هو عدوك الوحيد الآن!', 'error'); return redirect(request.referrer or url_for('home'))
    ...
    if item_type == 'weapon':
        ...
        if getattr(target, 'has_shield', False): attacker.update(pull__inventory=item_name, set__last_action_time=now); flash('الهدف محصن ضد التجسس!', 'error')
        ...
        if stolen_item in getattr(target, 'inventory', []):
            ...
            else: target.update(pull__inventory=stolen_item); attacker.update(push__inventory=stolen_item, pull__inventory=item_name, inc__intelligence_points=5, set__last_action_time=now); flash(f'تمت سرقة {stolen_item} بنجاح!', 'success')
    elif item_type == 'seal':
        if target.id == attacker.id:
            ...
            if item_name not in collected:
                ...
                if len(attacker.collected_seals) >= 4:
                    ...
                    User.objects(status='active').update(set__health=100); flash('🔥 جمعت الأختام الأربعة الفريدة! لقد فزت!', 'success')
                else: flash('فُعل الختم!', 'success')
            else: attacker.update(pull__inventory=item_name); flash('فعلت هذا النوع مسبقاً! تبخر الختم بلا فائدة.', 'error')
    return redirect(request.referrer or url_for('home'))
        ...
        if puzzle and str(guess) == str(getattr(puzzle, 'puzzle_answer', '')) and str(user.id) not in getattr(puzzle, 'winners_list', []):
            if getattr(puzzle, 'current_winners', 0) < getattr(puzzle, 'max_winners', 1):
                ...
                if settings and getattr(settings, 'floor1_mode_active', False):
                    user.update(inc__gems_collected=1); Notification(target_hunter_id=user.hunter_id, message='💎 حصلت على حجر كريم من اللغز!', notif_type='success').save()
                    if sum([u.gems_collected for u in User.objects(group_id=user.group_id)]) >= getattr(settings, 'floor1_gems_target', 10):
                        ...
                        GroupMessage(group_id=user.group_id, sender_name="النظام", message="✨ اكتمل النور وفُتحت البوابة!", is_system_msg=True).save(); User.objects(group_id=user.group_id, status='active').update(set__zone='الطابق 2')
...
def declarations():
    ...
    user.update(set__last_seen_decs=datetime.utcnow()); approved = News.objects(category='declaration', status='approved').order_by('-created_at'); pending = News.objects(category='declaration', status='pending') if user.role == 'admin' else []; my_pending = News.objects(category='declaration', status='pending', author=user.username).order_by('-created_at'); avatars = {u.username: u.hunter_id for u in User.objects(username__in=set([d.author for d in News.objects(category='declaration')]))}
...
@login_required
def store(): 
    g.user.update(set__last_seen_store=datetime.utcnow()); return render_template('store.html', items=StoreItem.objects())
...
def buy_item(item_id):
    ...
    if user and item and user.points >= item.price:
        if getattr(item, 'is_mirage', False): user.update(dec__points=item.price, dec__intelligence_points=10); flash(getattr(item, 'mirage_message', 'فخ سراب! خسرت الدنانير و 10 نقاط ذكاء.'), 'error')
        else:
            user.update(dec__points=item.price)
            if getattr(item, 'is_luck', False): outcome = random.randint(getattr(item, 'luck_min', 0), getattr(item, 'luck_max', 0)); user.update(inc__points=outcome); flash(f'صندوق حظ: النتيجة {outcome} دنانير!', 'success' if outcome >= 0 else 'error')
            else: user.update(push__inventory=item.name, inc__stats_items_bought=1); flash(f'تم شراء [{item.name}] بنجاح!', 'success')
...
def top_room():
    ...
    top_iq = User.objects(hunter_id__ne=1000, status__in=['active', 'inactive']).order_by('-intelligence_points')[:10]; top_loyal = User.objects(hunter_id__ne=1000, status__in=['active', 'inactive']).order_by('-loyalty_points')[:10]; top_hp = User.objects(hunter_id__ne=1000, status__in=['active', 'inactive']).order_by('-health')[:10]

...
def pre_process():
    ...
    if 'user_id' in session:
        try:
            ...
            # لا تمسح الجلسة مباشرة، فقط أعد التوجيه
        ...
        if user.hunter_id is None:
            ...
            for eid in existing_ids:
                if eid == new_id:
                    ...
                    break
            user.hunter_id = new_id
            user.save()
            flash('تم تحديث هويتك تلقائياً، يرجى تحديث الصفحة.', 'info')
            # إعادة التوجيه لتحديث الصفحة وضمان عرض ID الجديد
        ...
        if request.method == 'POST':
            ...
            if puzzle.puzzle_type in ['text', 'click_count', 'word_order']:
                ...
                if puzzle.puzzle_type == 'click_count':
                    # لغز الضغط المتكرر: الإجابة هي عدد مرات الضغط المطلوب

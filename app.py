...
app.config['SESSION_COOKIE_SECURE'] = False  # تم التعديل لحل مشكلة تسجيل الخروج في Render
...
def check_lazy_death_and_bleed(user, settings):
    try:
        ...
        # حماية 1000 و 1001 من النزيف والموت البطيء
...
def register():
    if request.method == 'POST':
        ...
        # خوارزمية ذكية لإعادة تدوير الأرقام (تبحث عن الفجوات وتبدأ من 1، متجاهلة 1000 ومساعديه)
...
def poneglyph():
    if getattr(g.user, 'role', '') != 'admin' and not getattr(g.user, 'unlocked_lore_room', False): 
        return redirect(url_for('home'))
    # Live Fetch لضمان ظهور النص فور التعديل من الإدارة
    fresh_settings = GlobalSettings.objects(setting_name='main_config').first()
...
def choose_gate():
    ...
    # اللاعب يختار فقط، ويدخل في الانتظار (لا يتحدد مصيره إلا من الإدارة)
...
def admin_panel():
        ...
        if act == 'toggle_gates':
            ...
            if spell_id:
                try:
                    ...
                    flash('خطأ في إبطال التعويذة. تأكد من أن الـ ID صحيح.', 'error')
    ...
    # تمرير دقيق لكافة المتغيرات لمنع خطأ 500
    return render_template('admin.html', users=users_query.order_by('-last_active')[:100], settings=settings, search_query=search_query, hidden_traps=hidden_traps, all_news=all_news, spells=spells, stats=stats, gate_counts=gate_counts)
...
# ==========================================
# 🎮 أوامر الطابق الأول (الاستنتاج - Among Us Style)
# ==========================================
...
def f1_move():
    ...
    if not new_room or new_room not in F1_MAP.get(user.current_room, []):
        flash('لا يمكنك الذهاب إلى هناك! هذه الغرفة غير متصلة بموقعك الحالي.', 'error')
        return redirect(url_for('home'))
        
    if getattr(settings, 'floor1_locked_room', '') == new_room:
        flash(f'⚠️ غرفة {new_room} مغلقة حالياً! لا يمكنك الدخول.', 'error')
        return redirect(url_for('home'))
    ...
    if last_move and (now - last_move).total_seconds() < cooldown:
        ...
        flash(f'⏳ يجب أن تنتظر {remaining} ثانية قبل التحرك مجدداً!', 'error')
        return redirect(url_for('home'))
...
def f1_complete_task(task_index):
    ...
    if not getattr(settings, 'floor1_mode_active', False) or user.status != 'active':
        flash('الطابق الأول غير نشط!', 'error')
        return redirect(url_for('home'))
    ...
    if task.get('completed', False):
        flash('⚠️ لقد أنجزت هذه المهمة مسبقاً!', 'error')
        return redirect(url_for('home'))
        
    if task.get('room') != user.current_room:
        flash(f'📍 هذه المهمة موجودة في {task["room"]}، أنت حالياً في {user.current_room}!', 'error')
        return redirect(url_for('home'))
    ...
    user.update(
        inc__points=reward,
        inc__gems_collected=1,
        set__f1_tasks=tasks
    )
    
    flash(f'✅ أنجزت مهمة "{task["name"]}" وحصلت على {reward} دينار و 💎 حجر كريم!', 'success')
    ...
    if total_gems >= gems_target:
        User.objects(group_id=user.group_id, is_cursed=True).update(set__status='eliminated', set__freeze_reason='احترقت اللعنة')
        GroupMessage(group_id=user.group_id, sender_name="النظام", message=f"✨ اكتمل جمع {gems_target} أحجار كريمة! الملعون احترق وفُتحت البوابة إلى الطابق الثاني! ✨", is_system_msg=True).save()
        User.objects(group_id=user.group_id, status='active').update(set__zone='الطابق 2')
        flash('🎉 لقد فازت مجموعتك! تم فتح البوابة إلى الطابق الثاني! 🎉', 'success')
...
def f1_emergency_meeting():
    ...
    if not getattr(settings, 'floor1_mode_active', False) or user.status != 'active':
        flash('لا يمكن عقد اجتماع الآن!', 'error')
        return redirect(url_for('home'))
    ...
    if gid_str in meetings:
        flash('⚠️ هناك اجتماع قائم بالفعل في مجموعتك!', 'error')
        return redirect(url_for('home'))
        
    if getattr(user, 'emergency_used', False):
        flash('🚫 لقد استخدمت زر الطوارئ مسبقاً! لا يمكنك استخدامه مرة أخرى.', 'error')
        return redirect(url_for('home'))
    ...
    meetings[gid_str] = {
        ...
        'type': 'emergency',
        'caller_hunter_id': user.hunter_id
    }
    
    GlobalSettings.objects(setting_name='main_config').update_one(set__f1_active_meetings=meetings)
    user.update(set__emergency_used=True)
    
    GroupMessage(group_id=gid, sender_name="النظام", message=f"🚨 {user.username} استدعى اجتماعاً طارئاً! التصويت متاح لمدة 5 دقائق. 🚨", is_system_msg=True).save()
    
    flash('🚨 تم عقد الاجتماع الطارئ! توجه إلى غرفة الاجتماعات للتصويت.', 'success')
    return redirect(url_for('home'))
...
def f1_report_body():
    ...
    if not getattr(settings, 'floor1_mode_active', False) or user.status != 'active':
        flash('لا يمكنك الإبلاغ الآن!', 'error')
        return redirect(url_for('home'))
    ...
    if gid_str in meetings:
        flash('⚠️ هناك اجتماع قائم بالفعل!', 'error')
        return redirect(url_for('home'))
        
    body_hunter_id = int(request.form.get('body_id', 0))
    body = User.objects(hunter_id=body_hunter_id, group_id=gid, status='dead_body').first()
    
    if not body:
        flash('❌ لا توجد جثة هنا للإبلاغ عنها!', 'error')
        return redirect(url_for('home'))
    ...
    meetings[gid_str] = {
        ...
        'type': 'report',
        'body': body.username,
        'body_hunter_id': body.hunter_id,
        'caller_hunter_id': user.hunter_id
    }
    
    GlobalSettings.objects(setting_name='main_config').update_one(set__f1_active_meetings=meetings)
    
    GroupMessage(group_id=gid, sender_name="النظام", message=f"⚠️ {user.username} عثر على جثة {body.username} وطلب اجتماعاً طارئاً! ⚠️", is_system_msg=True).save()
    
    flash(f'⚠️ أبلغت عن جثة {body.username}! تم عقد اجتماع طارئ.', 'success')
    return redirect(url_for('home'))
...
def f1_kill():
    ...
    if not getattr(settings, 'floor1_mode_active', False) or attacker.status != 'active':
        flash('لا يمكنك القتل الآن!', 'error')
        return redirect(url_for('home'))
        
    if not getattr(attacker, 'is_cursed', False):
        flash('❌ أنت لست الملعون! لا يمكنك القتل.', 'error')
        return redirect(url_for('home'))
    ...
    if not target:
        flash('❌ الهدف غير موجود أو ليس في نفس الغرفة!', 'error')
        return redirect(url_for('home'))
    ...
    if last_kill and (now - last_kill).total_seconds() < kill_cooldown:
        remaining = int(kill_cooldown - (now - last_kill).total_seconds())
        flash(f'⏳ لا يمكنك القتل مجدداً! انتظر {remaining} ثانية.', 'error')
        return redirect(url_for('home'))
    ...
    for player in User.objects(group_id=attacker.group_id, current_room=attacker.current_room, status='active'):
        if player.id != attacker.id:
            Notification(target_hunter_id=player.hunter_id, message=f'💀 سمعت صراخاً في الغرفة... يبدو أن {target.username} قد قُتل! 💀', notif_type='danger').save()
    ...
    flash(f'🔪 قتلت {target.username} بنجاح! اختفى الجسد...', 'success')
    return redirect(url_for('home'))
...
def f1_vent():
    ...
    if not getattr(settings, 'floor1_mode_active', False) or user.status != 'active':
        flash('لا يمكنك استخدام النفق الآن!', 'error')
        return redirect(url_for('home'))
        
    if not getattr(user, 'is_cursed', False):
        flash('❌ فقط الملعون يمكنه استخدام الأنفاق السرية!', 'error')
        return redirect(url_for('home'))
        
    if getattr(user, 'used_vent', False):
        flash('🚫 لقد استخدمت النفق السري مسبقاً! لا يمكنك استخدامه مرة أخرى.', 'error')
        return redirect(url_for('home'))
        
    vent_rooms = ['دهليز الأجداد', 'مخزن الآثار']
    if user.current_room not in vent_rooms:
        flash('📍 لا يوجد نفق سري في هذه الغرفة!', 'error')
        return redirect(url_for('home'))
    ...
    flash(f'🕳️ تسللت عبر النفق السري وظهرت فجأة في {new_room}! (استخدمت النفق مرة واحدة فقط)', 'success')
    return redirect(url_for('home'))
...
def f1_vote():
    ...
    if gid_str not in meetings:
        flash('❌ لا يوجد اجتماع في مجموعتك حالياً!', 'error')
        return redirect(url_for('home'))
        
    if getattr(user, 'f1_has_voted', False):
        flash('⚠️ لقد صوت مسبقاً! لا يمكنك تغيير صوتك.', 'error')
        return redirect(url_for('home'))
    ...
    if not target or target.id == user.id:
        flash('❌ هدف غير صالح للتصويت!', 'error')
        return redirect(url_for('home'))
...
def f3_cast_vote():
    ...
    if target.hunter_id in [1000, 1001]:
        flash('لا يمكن التصويت ضد هذا الكيان!', 'error')
        return redirect(url_for('home'))
...
def f3_results():
    ...
    winners = User.objects(status='active', role='hunter', zone='المعركة الأخيرة', hunter_id__nin=[1000, 1001]).order_by('-survival_votes')
...
def submit_floor3_votes():
    ...
    for i in range(1, 6):
        ...
        if not target or target.hunter_id in [1000, 1001]:
            flash('أحد الأهداف غير موجود أو محصن!', 'error')
            return redirect(url_for('home'))

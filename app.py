...
def check_lazy_death_and_bleed(user, settings):
    ...
    hours_passed = (now - last_act).total_seconds() / 3600.0
    if hours_passed > 72:
    ...
    if settings and getattr(settings, 'war_mode', False):
        ...
        safe_mins = getattr(settings, 'safe_time_minutes', 120) or 120
        safe_until = last_action + timedelta(minutes=safe_mins)
        
        if now > safe_until:
            last_health_chk = getattr(user, 'last_health_check', None) or safe_until
            start_bleed_time = max(last_health_chk, safe_until)
            minutes_passed = (now - start_bleed_time).total_seconds() / 60.0
            
            bleed_rate = getattr(settings, 'bleed_rate_minutes', 60) or 60
            bleed_amount = getattr(settings, 'bleed_amount', 1) or 1
            ...
            if bleed_rate > 0 and minutes_passed >= bleed_rate:
                ...
                if user.health <= 0:
                    ...
                    settings.dead_count = (getattr(settings, 'dead_count', 0) or 0) + 1; settings.save()
                user.save()
...
def check_locks_and_status():
    ...
    if user:
        ...
        lock_until = getattr(user, 'quicksand_lock_until', None)
        if lock_until and datetime.utcnow() < lock_until:
            time_left = lock_until - datetime.utcnow()
        ...
        if getattr(user, 'gate_status', '') == 'testing' and request.endpoint not in ['submit_gate_test', 'logout']:
            return render_template('gate_test.html', message=getattr(settings, 'gates_test_message', 'الاختبار'))
...
def home():
    ...
    active_voters_count = User.objects(status='active', role='hunter', has_voted=True).count() if settings and getattr(settings, 'floor3_mode_active', False) else 0
...
def choose_gate():
    ...
    settings = GlobalSettings.objects(setting_name='main_config').first()
    if not getattr(settings, 'gates_mode_active', False) or getattr(settings, 'gates_selection_locked', False): return redirect(url_for('home'))
    gate_num = int(request.form.get('gate_num', 0) or 0)
    if gate_num in [1, 2, 3] and getattr(user, 'chosen_gate', 0) == 0:
...
def submit_gate_test():
    user = User.objects(id=session['user_id']).first()
    if getattr(user, 'gate_status', '') == 'testing': user.gate_test_answer = request.form.get('test_answer', ''); user.save()
    return redirect(url_for('home'))
...
def submit_floor3_votes():
    user = User.objects(id=session['user_id']).first()
    if user.status != 'active' or getattr(user, 'has_voted', False): return redirect(url_for('home'))
    try:
        t_ids = [int(request.form.get(f'target_{i}', 0) or 0) for i in range(1, 6)]
        amounts = [int(request.form.get(f'amount_{i}', 0) or 0) for i in range(1, 6)]
    ...
    for i, tid in enumerate(t_ids):
        t = User.objects(hunter_id=tid).first(); t.survival_votes = getattr(t, 'survival_votes', 0) + amounts[i]; t.save()
...
def register():
    if request.method == 'POST':
        if User.objects(username=request.form['username']).first(): flash('مستخدم.', 'error'); return redirect(url_for('register'))
...
def update_settings():
    ...
    if action == 'change_avatar':
        ...
        if last_change and (now - last_change).days < 15: flash('كل 15 يوم!', 'error')
        elif User.objects(username=new_name).first(): flash('مستخدم!', 'error')
...
def admin_update_profile(target_id):
    ...
    if target_user:
        ...
        elif action == 'edit_points': target_user.points = int(request.form.get('new_points', 0) or 0); flash('تم التعديل!', 'success')
        elif action == 'edit_hp':
            target_user.health = int(request.form.get('new_hp', 0) or 0)
...
def transfer(target_id):
    ...
    if ttype == 'points':
        amt = int(request.form.get('amount', 0) or 0)
        if 0 < amt <= sender.points: sender.points -= amt; receiver.points += amt; sender.save(); receiver.save(); flash('تُحول!', 'success')
    elif ttype == 'item':
        itm = request.form.get('item_name')
        if itm in sender.inventory: sender.inventory.remove(itm); receiver.inventory.append(itm); sender.save(); receiver.save(); flash('أُرسل!', 'success')
...
def use_item(target_id):
    ...
    if getattr(item, 'item_type', '') == 'weapon':
        if getattr(settings, 'war_mode', False) and target.hunter_id not in attacker.friends:
            ...
            if target.health <= 0: target.health = 0; target.status = 'eliminated'; settings.dead_count = getattr(settings, 'dead_count', 0) + 1; settings.save()
        ...
        if target.id == attacker.id or target.hunter_id in attacker.friends:
            if getattr(target, 'role', '') == 'admin': target.health += getattr(item, 'effect_amount', 0)
            else: target.health = min(100, target.health + getattr(item, 'effect_amount', 0))
...
def add_friend():
    ...
    target = User.objects(hunter_id=int(request.form.get('target_id', 0) or 0)).first()
    ...
    if getattr(target, 'role', '') in ['ghost', 'cursed_ghost']:
        ...
        if getattr(target, 'role', '') == 'cursed_ghost' and trap: user.points -= getattr(trap, 'trap_penalty_points', 0); user.save(); flash('أيقظت شبحاً ملعوناً!', 'error')
            ...
            user.stats_ghosts_caught = getattr(user, 'stats_ghosts_caught', 0) + 1; trap.current_winners += 1; trap.winners_list.append(str(user.id))
...
def puzzles():
    ...
    if request.method == 'POST':
        ...
        if puzzle and guess == puzzle.puzzle_answer and str(user.id) not in puzzle.winners_list:
            user.points += getattr(puzzle, 'reward_points', 0); user.stats_puzzles_solved = getattr(user, 'stats_puzzles_solved', 0) + 1; puzzle.winners_list.append(str(user.id))
...
def secret_link(puzzle_id):
    ...
    if puzzle and getattr(puzzle, 'puzzle_type', '') == 'quicksand_trap':
        ...
        user.points += getattr(puzzle, 'reward_points', 0); puzzle.current_winners += 1; puzzle.winners_list.append(str(user.id))
...
def buy_item(item_id):
    ...
    if user and item and user.points >= item.price:
        ...
        if getattr(item, 'is_luck', False):
            luck_min = getattr(item, 'luck_min', 0) or 0
            luck_max = getattr(item, 'luck_max', 0) or 0
            if luck_min > luck_max: luck_min, luck_max = luck_max, luck_min
            outcome = random.randint(luck_min, luck_max) if luck_min != luck_max else luck_min
            user.points += outcome
            ...
            flash(getattr(item, 'mirage_message', 'فخ السراب!'), 'error')
        else:
...
def admin_panel():
    ...
    if request.method == 'POST':
        ...
        if action == 'setup_maintenance':
            ...
            admin_user = User.objects(id=session['user_id']).first(); admin_user.health = int(request.form.get('admin_hp', 100) or 100); admin_user.save(); flash('💪 تم رفع صحتك الإمبراطورية!', 'success')
        ...
        elif action == 'update_war_settings': settings.bleed_rate_minutes = int(request.form.get('bleed_rate_minutes', 60) or 60); settings.bleed_amount = int(request.form.get('bleed_amount', 1) or 1); settings.safe_time_minutes = int(request.form.get('safe_time_minutes', 120) or 120); settings.save()
            ...
            item = StoreItem(name=request.form.get('item_name'), description=request.form.get('item_desc'), price=int(request.form.get('item_price', 0) or 0), item_type=request.form.get('item_type', 'normal'), effect_amount=int(request.form.get('effect_amount', 0) or 0), is_mirage=bool(request.form.get('is_mirage')), mirage_message=request.form.get('mirage_message', ''), is_luck=bool(request.form.get('is_luck')), luck_min=int(request.form.get('luck_min', 0) or 0), luck_max=int(request.form.get('luck_max', 0) or 0))
            file = request.files.get('item_image')
            if file and file.filename != '': item.image = f"data:{file.content_type};base64,{base64.b64encode(file.read()).decode('utf-8')}"
            item.save(); flash('تم إضافة الأداة للسوق!', 'success')

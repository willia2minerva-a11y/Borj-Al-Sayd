...
def altar():
    ...
    if request.method == 'POST':
        ...
        # 🚨 الحماية من المؤقت، العدد المحدود، والتكرار
        now = datetime.utcnow()
...
# 🚀 مسار بونغليف سيفار (بديل السجلات)
@app.route('/poneglyph')
@login_required
def poneglyph():
    user = User.objects(id=session['user_id']).exclude('avatar').first()
...
def admin_panel():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # 🚀 تم تحديث إضافة التعويذة لتشمل المؤقت والعدد
            elif act == 'add_spell': 
                ...
                # 🚀 تنظيف أي سجلات قديمة مخفية نهائياً لتسريع قاعدة البيانات
                db.connection[app.config['MONGODB_SETTINGS']['host'].split('/')[-1]]['action_log'].drop()
                db.connection[app.config['MONGODB_SETTINGS']['host'].split('/')[-1]]['lore_log'].drop()
                flash('تم مسح جميع السجلات من جذورها لتخفيف الموقع!', 'success')
        except: pass

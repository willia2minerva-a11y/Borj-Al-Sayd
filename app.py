...
def compress_image(image_data, quality=70, max_size=(600, 600)):
...
def execute_trap_effect(user, trap):
    ...
    if eff == 'give_points': user.update(inc__points=trap.reward_points, inc__intelligence_points=10); flash(f'نجاح! حصلت على {trap.reward_points} دنانير!', 'success')
        ...
        seal_name = 'ختم الغموض' if trap.puzzle_type.startswith('link') else 'ختم الأرواح'
        if seal_name not in user.collected_seals:
            user.update(push__inventory=seal_name); flash(f'نجاح أسطوري! اكتشفت {seal_name}!', 'success')
...
def pre_process():
    ...
    if settings:
        ...
        if not settings.sleep_mode_active:
            ...
            # البوابات
            if getattr(settings, 'gates_mode_active', False) and settings.gates_end_time and now >= settings.gates_end_time:
            ...
            # الطابق الثالث
            if getattr(settings, 'floor3_mode_active', False) and not getattr(settings, 'floor3_paused', False) and settings.vote_end_time and now >= settings.vote_end_time:
                ...
                for u in User.objects(status='active', role='hunter', hunter_id__nin=[1000, 1001]).order_by('-survival_votes')[:getattr(settings, 'vote_top_n', 5)]: 
        ...
        if getattr(settings, 'maintenance_mode', False) and getattr(settings, 'maintenance_until', None) and now > settings.maintenance_until:
            GlobalSettings.objects(setting_name='main_config').update_one(set__maintenance_mode=False, set__maintenance_pages=[])
        if getattr(settings, 'maintenance_mode', False) and not user_is_admin:
...
def home():
    ...
    active_hunters = User.objects(status='active', role='hunter', hunter_id__nin=[1000, 1001], id__ne=user.id if user else None) if settings and getattr(settings, 'floor3_mode_active', False) else []
...
def update_settings():
    ...
    try:
        if action == 'change_avatar':
            ...
            if getattr(user, 'last_name_change', None) and (now - user.last_name_change).days < 7: flash('تغيير الاسم متاح مرة كل 7 ايام!', 'error')
    ...
    except Exception as e: flash('حدث خطأ في تحديث البيانات.', 'error')
    return redirect(url_for('profile'))

@app.route('/hunter/<int:target_id>')
@login_required
def hunter_profile(target_id):
    if target_id in [1000, 1001] and getattr(g.user, 'hunter_id', 0) not in [1000, 1001]:
    ...
    # التحقق من وجود "عين سيفار" مفعلة لكشف الملف
    can_see_hidden = False
    if getattr(g.user, 'tajis_eye_until', None) and datetime.utcnow() < getattr(g.user, 'tajis_eye_until'):
        can_see_hidden = True
    ...
    return render_template(
        ...
        my_spies=[i for i in my_items if getattr(i, 'item_type', '') in ['spy', 'sephar_eye']], 
        my_steals=[i for i in my_items if getattr(i, 'item_type', '') in ['steal', 'sephar_hand']],
...
def friends():
    ...
    if search_query: 
        ...
        elif search_query.isdigit(): search_result = User.objects(hunter_id=int(search_query)).first()
        else: search_result = User.objects(username__icontains=search_query, hunter_id__nin=[1000, 1001]).first()
...
def add_friend():
    ...
    if user.status != 'active' or not target or target.status not in ['active', 'inactive']: return redirect(request.referrer or url_for('home'))
...
# ==========================================
# 🎁 التفاعلات والقتال والأدوات (وشاح، عين، يد سيفار والأختام)
# ==========================================
...
def transfer(target_id):
    ...
    if sender.status != 'active' or not receiver: return redirect(request.referrer or url_for('home'))
...
def use_item(target_id):
    ...
    # ميكانيكية تحطيم الأختام الخاصة (لا تحتاج لوجودها في المتجر)
    seals_list = ['ختم الهمسات', 'ختم الجشع', 'ختم الأرواح', 'ختم الغموض']
    if item_name in seals_list and item_name in getattr(attacker, 'inventory', []):
        if attacker.id != target.id: 
            flash('يجب تحطيم الأختام على روحك أنت لدمجها!', 'error')
            return redirect(request.referrer or url_for('profile'))
        
        if item_name not in getattr(attacker, 'collected_seals', []):
            attacker.update(push__collected_seals=item_name, pull__inventory=item_name)
            attacker.reload()
            if len(attacker.collected_seals) >= 4:
                # تدمير الإمبراطور وإعلان الفوز
                emperor = User.objects(hunter_id=1000).first()
                if emperor: emperor.update(set__health=0)
                GlobalSettings.objects(setting_name='main_config').update_one(set__war_mode=False, set__final_battle_mode=False, set__maze_winner_id=attacker.hunter_id)
                User.objects(status='active', hunter_id__nin=[1000, 1001, attacker.hunter_id]).update(set__health=0, set__status='eliminated', set__freeze_reason='سُحقت الروح بقوة الأختام الأربعة')
                attacker.update(set__health=100)
                flash('🔥 حطمت الأختام الأربعة الفريدة! الإمبراطور سقط.. لقد دمرت المتاهة وتوجت بالعرش!', 'success')
            else:
                flash(f'تم تحطيم {item_name} ودمجه بروحك بنجاح!', 'success')
        else:
            attacker.update(pull__inventory=item_name)
            flash('لقد دمجت طاقة هذا الختم مسبقاً! تبخر بلا فائدة.', 'error')
        return redirect(request.referrer or url_for('profile'))

    # مسار الأدوات العادية
    ...
    if attacker.status != 'active' and item_type != 'totem': 
        flash('حسابك معطل، الأطياف لا يمكنها الهجوم!', 'error'); return redirect(request.referrer or url_for('home'))

    if item_type == 'totem':
        if not getattr(settings, 'war_mode', False): flash('التوتم لا يُفعل إلا في الحرب الشاملة!', 'error')
        elif target.status not in ['eliminated', 'dead_body']: flash('اللاعب لا يزال حياً يُرزق!', 'error')
        elif target.hunter_id in [1000, 1001]: flash('لا يمكن استخدامه على الكيانات الإمبراطورية!', 'error')
        else:
            target.update(set__status='active', set__health=50, set__freeze_reason='')
            attacker.update(pull__inventory=item_name)
            if attacker.id == target.id: flash('🌟 انتفضت الروح! أعدت نفسك للحياة بنصف قوتك!', 'success')
            else: flash(f'🌟 لقد منحت روحك للرحالة {target.username}!', 'success')
        return redirect(request.referrer or url_for('home'))

    if not target or target.status != 'active': return redirect(request.referrer or url_for('home'))

    # التبريد للأدوات الهجومية والسرقة والتجسس
    if item_type in ['weapon', 'steal', 'sephar_hand', 'spy', 'sephar_eye']:
        cooldown_mins = getattr(settings, 'attack_cooldown_minutes', 0)
        if cooldown_mins > 0 and getattr(attacker, 'last_action_time', None) and (now - attacker.last_action_time).total_seconds() < (cooldown_mins * 60):
            flash(f'⏳ انتظر {int((cooldown_mins*60) - (now - attacker.last_action_time).total_seconds())} ثانية.', 'error')
            return redirect(request.referrer or url_for('home'))

    if getattr(settings, 'final_battle_mode', False) and target.hunter_id != 1000 and item_type in ['weapon']: 
        flash('الإمبراطور هو عدوك الوحيد الآن! وجّه ضربتك نحو العرش!', 'error'); return redirect(request.referrer or url_for('home'))

    # دفاع وشاح سيفار
    is_target_protected = False
    if 'وشاح سيفار' in getattr(target, 'inventory', []):
        is_target_protected = True
        target.update(pull__inventory='وشاح سيفار')
        if item_type in ['weapon', 'steal', 'sephar_hand', 'spy', 'sephar_eye']:
            attacker.update(dec__health=10, pull__inventory=item_name, set__last_action_time=now)
            flash('الهدف يرتدي وشاح سيفار! احترقت يدك وفقدت 10 من صحتك وضاعت أداتك!', 'error')
            return redirect(request.referrer or url_for('home'))
    ...
    # عين سيفار / تجسس
    elif item_type in ['spy', 'sephar_eye']:
        attacker.update(set__tajis_eye_until=now + timedelta(minutes=15), pull__inventory=item_name, set__last_action_time=now)
        flash('تم تفعيل عين سيفار! يمكنك رؤية خبايا الهدف لـ 15 دقيقة.', 'success')
            
    # يد سيفار / سرقة
    elif item_type in ['steal', 'sephar_hand']:
        target_inventory = getattr(target, 'inventory', [])
        if target_inventory:
            ...
            flash(f'تمت سرقة [{stolen_item}] بواسطة يد سيفار بنجاح!', 'success')
        else:
            attacker.update(pull__inventory=item_name, set__last_action_time=now)
            flash('حقيبة الهدف فارغة! ضاعت محاولتك.', 'error')

    return redirect(request.referrer or url_for('home'))

# ==========================================
# 🕵️ محرك الطابق الأول (الاستنتاج)
# ==========================================
...
def f1_task():
    ...
    for t in tasks:
        if t.get('name') == task_name and t.get('room') == user.current_room and not t.get('completed', False):
            t['completed'] = True; updated = True; break
...
def f1_kill():
    ...
    if occupants > 2: flash('⚠️ احذر! لقد قتلت بوجود شهود! جريمتك علنية ومفضوحة!', 'error')
...
def declarations():
    ...
    if request.method == 'POST':
        ...
        flash('تم إرسال التصريح بنجاح!', 'success')
...
@app.route('/approve_declaration/<dec_id>', methods=['POST'])
@admin_required
def approve_declaration(dec_id):
    dec = News.objects(id=ObjectId(dec_id)).first()
    if dec: dec.update(set__status='approved'); flash('تمت الموافقة على التصريح.', 'success')
    return redirect(url_for('declarations'))
...
def delete_declaration(dec_id):
    dec = News.objects(id=ObjectId(dec_id)).first()
    if dec and (g.user.role == 'admin' or dec.author == g.user.username): dec.delete(); flash('تم طمس التصريح.', 'success')
    return redirect(url_for('declarations'))
...
def react_declaration(dec_id):
    dec = News.objects(id=ObjectId(dec_id)).first()
    if dec and str(g.user.id) not in getattr(dec, 'reactions', []): dec.update(push__reactions=str(g.user.id))
    return redirect(url_for('declarations'))
...
def use_spell():
    ...
    if getattr(spell, 'expires_at', None) and datetime.utcnow() > spell.expires_at:
        flash('هذه التعويذة فقدت طاقتها الزمنية!', 'error'); return redirect(url_for('altar'))
        
    if len(spell.used_by) >= spell.max_uses and str(user.id) not in spell.used_by:
        flash('استُنزفت طاقة هذه التعويذة من قبل رحالة آخرين!', 'error'); return redirect(url_for('altar'))
        
    if str(user.id) in spell.used_by:
        flash('لقد استخدمت هذه التعويذة مسبقاً!', 'info'); return redirect(url_for('altar'))

    # تأثيرات التعاويذ المفصولة تماماً
    st = spell.spell_type
    ...
    elif st == 'give_item': user.update(push__inventory=spell.item_name); flash(f'تجسد لك: {spell.item_name}!', 'success')
    elif st == 'give_seal_whispers': 
        if 'ختم الهمسات' not in getattr(user, 'collected_seals', []):
            user.update(push__inventory='ختم الهمسات'); flash('لقد اكتشفت ختم الهمسات الأسطوري!', 'success')
...
# ==========================================
# 👑 لوحة الإدارة المركزية المعصومة ضد ValueError
# ==========================================
...
def admin_delete_item():
    tid = int(request.form.get('target_id') or 0)
...
def admin_panel():
    ...
    if request.method == 'POST':
        ...
        if act == 'hard_delete_single':
            ...
            if settings.sleep_mode_active:
                ...
                GlobalSettings.objects(setting_name='main_config').update_one(set__maintenance_mode=True, set__maintenance_pages=[request.form.get('maint_page', 'all')], set__maintenance_until=datetime.utcnow() + timedelta(hours=int(request.form.get('maint_hours') or 1)))
            ...
            if getattr(settings, 'gates_mode_active', False):
                ...
                GlobalSettings.objects(setting_name='main_config').update_one(
                    ...
                    set__gates_test_question=request.form.get('test_question', 'ما هو سر المتاهة؟')
                )
            ...
            if getattr(settings, 'floor1_mode_active', False):
                ...
                gs = int(request.form.get('group_size') or 5)
                group_id = 1
                ...
                GlobalSettings.objects(setting_name='main_config').update_one(
                    ...
                    set__floor1_move_cooldown=int(request.form.get('move_cooldown') or 30), 
                    set__floor1_kill_cooldown=int(request.form.get('kill_cooldown') or 60), 
                    set__floor1_individual_gems_target=int(request.form.get('gems_target') or 3), 
                    set__f1_cursed_win_percent=int(request.form.get('f1_cursed_win_percent') or 50), 
            ...
            if getattr(settings, 'floor3_mode_active', False):
                ...
                GlobalSettings.objects(setting_name='main_config').update_one(set__floor3_mode_active=True, set__floor3_results_active=False, set__vote_end_time=datetime.utcnow() + timedelta(hours=vh) if vh > 0 else None, set__vote_top_n=int(request.form.get('top_n') or 5))
                flash('بدأت المحكمة الكبرى!', 'success')

        elif act == 'reset_floor3':
            GlobalSettings.objects(setting_name='main_config').update_one(set__floor3_mode_active=False, set__floor3_results_active=False, set__floor3_paused=False)
            User.objects.update(set__has_voted=False, set__survival_votes=0.0, set__f3_votes_cast={}, set__f3_vote_target=0)
            flash('تم تصفير محكمة التصويت بالكامل!', 'success')
        ...
        # إضافة عنصر جديد للسوق محصنة ضد ValueError
            ...
            StoreItem(
                ...
                mirage_message=request.form.get('mirage_msg', ''), 
            ...
            News(
                title="كيان", content="خفي", category='hidden', 
                puzzle_type=f"{cat}_{request.form.get('trap_effect')}", 
                puzzle_answer=ans, 
            ...
            if spell_id:
                ...
                flash('تم إبطال التعويذة بنجاح!', 'success')
                
        elif act == 'add_news':
            img = ''
            file = request.files.get('image_file')
            if file and file.filename != '': 
                img = f"data:{file.content_type};base64,{base64.b64encode(compress_image(file.read())).decode('utf-8')}"
            
            p_type = request.form.get('puzzle_type', 'none')
            cat = 'puzzle' if p_type != 'none' else 'news'
            ...
            flash('تم نشر المرسوم/اللغز بنجاح!', 'success')
    ...
    gate_counts = {1: User.objects(status='active', chosen_gate=1, hunter_id__nin=[1000, 1001]).count(), 2: User.objects(status='active', chosen_gate=2, hunter_id__nin=[1000, 1001]).count(), 3: User.objects(status='active', chosen_gate=3, hunter_id__nin=[1000, 1001]).count(), 0: User.objects(status='active', chosen_gate=0, hunter_id__nin=[1000, 1001]).count()}
    store_items = [i.name for i in StoreItem.objects()]
    
    return render_template('admin.html', users=users_query.order_by('-last_active')[:100], settings=settings, search_query=search_query, hidden_traps=News.objects(category='hidden'), all_news=News.objects(category__in=['news', 'puzzle', 'declaration']).order_by('-created_at'), spells=SpellConfig.objects().order_by('-created_at'), stats=stats, gate_counts=gate_counts, store_items=store_items)

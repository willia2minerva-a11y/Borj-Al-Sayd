...
# 📜 المهام المتاحة (مهمتان أو ثلاث في كل غرفة)
TASKS_BY_ROOM = {
    'قاعة العروش': [
...
def assign_tasks_to_player(user):
    """توزيع 3 مهام عشوائية للاعب من جميع المهام المتاحة"""
    all_tasks = []
    ...
    if settings and getattr(settings, 'floor1_mode_active', False) and user and user.status == 'active':
        ...
        if user.current_room == 'غرفة البصيرة':
            for u in User.objects(group_id=user.group_id, status__in=['active', 'dead_body']):
                if u.current_room not in admin_dots:
                    admin_dots[u.current_room] = []
            ...
            for u in User.objects(group_id=user.group_id, status='active', current_room__in=monitored_rooms):
                if u.current_room not in camera_feeds: 
                    camera_feeds[u.current_room] = []
...
def add_friend():
    ...
    if getattr(target, 'role', '') in ['ghost', 'cursed_ghost']:
        ...
        if trap and str(user.id) not in getattr(trap, 'winners_list', []) and getattr(trap, 'current_winners', 0) < getattr(trap, 'max_winners', 1):
            ...
            flash('اختفى الشبح أو نفدت مكافآته.', 'info')
...
def admin_update_profile(target_id):
    ...
    if target_user:
        try:
            ...
            if action == 'edit_name': 
                target_user.update(set__username=val)
            elif action == 'edit_points': 
                target_user.update(set__points=int(val or 0))
            elif action == 'edit_hp': 
                target_user.update(set__health=int(val or 0), set__status='eliminated' if int(val or 0) <= 0 else 'active')
            elif action == 'edit_iq': 
                target_user.update(set__intelligence_points=int(val or 0))
            elif action == 'edit_loyalty': 
                target_user.update(set__loyalty_points=int(val or 0))
            elif action == 'edit_zone': 
                target_user.update(set__zone=val)
...
def admin_panel():
    ...
    if request.method == 'POST':
        ...
        if act == 'toggle_gates':
            ...
            for uid in selected_users:
                ...
                if u and u.hunter_id != 1000:
                    if bt == 'hard_delete': 
                        ...
                        u.update(set__status='active', set__health=100)
                    elif bt == 'eliminate': 
                        u.update(set__status='eliminated', set__freeze_reason='قرار إداري')
                    elif bt == 'freeze': 
                        u.update(set__status='frozen')
                    elif bt == 'move_zone': 
                        u.update(set__zone=request.form.get('bulk_zone', 'الطابق 1'))
            ...
            if file and file.filename != '': 
                compressed_banner = compress_image(file.read())
                GlobalSettings.objects(setting_name='main_config').update_one(
                    set__banner_url=f"data:{file.content_type};base64,{base64.b64encode(compressed_banner).decode('utf-8')}"
                )
            ...
            News(
                ...
                category='puzzle' if pt != 'none' else 'news', 
                puzzle_type=pt, 
                puzzle_answer=ans, 
                reward_points=int(request.form.get('reward_points') or 0), 
                max_winners=int(request.form.get('max_winners') or 1), 
            ...
            try: 
                SpellConfig.objects(id=request.form.get('spell_id')).delete()
    ...
    # 🔍 تجهيز المتغيرات لعرضها في صفحة الإدارة

...
app.config['SECRET_KEY'] = 'sephar-maze-emperor-ultimate-v9-spells'
...
def log_action(text, cat, is_epic=False):
    try: 
        ...
        # إضافة للسجلات القصصية (Lore)
        if is_epic:
            LoreLog(content=text, is_epic=True).save()
        elif random.random() < 0.3: # 30% من الأحداث تتحول لقصص تاريخية
            LoreLog(content=text).save()
    except: pass
            ...
            if now > safe_until:
                ...
                if bleed_rate > 0 and minutes_passed >= bleed_rate:
                    ...
                    if user.health <= 0:
                        ...
                        # 🛑 نظام ضريبة الدم (إيقاف الحرب إذا تحقق العدد المطلوب)
                        if getattr(settings, 'war_mode', False) and settings.dead_count >= getattr(settings, 'war_kill_target', 15):
                            settings.war_mode = False
                            log_action(f"🛑 اكتفت المتاهة من الدماء. سقط {settings.dead_count} ضحية، وتوقفت الحرب الشاملة لتفتح محكمة الأصوات أبوابها!", "system", is_epic=True)
...
# ==============================================================
# هنا ينتهي الجزء الأول، وسأعطيك مسارات النظام والمذبح في الجزء الثاني

# --- مسارات الواجهة والتسجيل ---
...
# --- نظام المعارك المتقدم (مع الدروع والطوطم) ---
...
def use_item(target_id):
    ...
    if getattr(item, 'item_type', '') == 'weapon' and is_combat_active and target.hunter_id not in getattr(attacker, 'friends', []):
        ...
        if has_shield:
            ...
            if target.health <= 0: 
                # 🌟 نظام الطوطم (يعمل فقط إذا لم تكن المعركة الأخيرة أو التصويت مفعلين)
                has_totem = False
                ...
                if has_totem:
                    target.health = 50 # يعود بنصف صحته
...
def altar():
    ...
    if request.method == 'POST':
        ...
        lore_msg = getattr(spell, 'lore_message', f"الرحالة {user.username} تلا تعويذة منسية!")
        
        # تنفيذ تأثير التعويذة
        if stype == 'hp_loss':
...
def add_friend():
    ...
    if getattr(target, 'role', '') in ['ghost', 'cursed_ghost']:
        ...
        if getattr(target, 'role', '') == 'cursed_ghost' and trap: 
            ...
            trap.current_winners = getattr(trap, 'current_winners', 0) + 1; trap.winners_list.append(str(user.id))
...
def puzzles():
    ...
    if request.method == 'POST':
        if getattr(user, 'status', '') != 'active': 
            flash('حسابك يحتاج تفعيل للمشاركة.', 'error'); return redirect(url_for('puzzles'))
        ...
        if puz and guess == getattr(puz, 'puzzle_answer', '') and str(user.id) not in getattr(puz, 'winners_list', []):
            if getattr(puz, 'current_winners', 0) < getattr(puz, 'max_winners', 1):
                ...
                puz.winners_list.append(str(user.id)); puz.current_winners = getattr(puz, 'current_winners', 0) + 1
...
def declarations():
    ...
    if request.method == 'POST':
        if getattr(user, 'status', '') != 'active': 
            flash('حسابك قيد المراجعة ولا يسمح لك بالنشر.', 'error'); return redirect(url_for('declarations'))
    ...
    users_query = User.objects(username__in=authors).only('username', 'avatar')
    avatars = {u.username: getattr(u, 'avatar', None) for u in users_query}
...
def buy_item(item_id):
    ...
    if user and item and user.points >= item.price:
        ...
        if getattr(item, 'is_luck', False):
            ...
            user.intelligence_points -= 10
            log_action(f"🕸️ وقع {user.username} في فخ المتجر", "puzzle"); flash(getattr(item, 'mirage_message', 'فخ!'), 'error')
        else: 
    ...
    if request.method == 'POST':
        ...
        try:
            if act == 'moderate_dec':
                ...
                SpellConfig(spell_word=request.form.get('spell_word'), spell_type=request.form.get('spell_type'), effect_value=int(request.form.get('effect_value') or 0), is_percentage=bool(request.form.get('is_percentage')), item_name=request.form.get('item_name', ''), lore_message=request.form.get('lore_message', '[user] ألقى تعويذة غامضة')).save()

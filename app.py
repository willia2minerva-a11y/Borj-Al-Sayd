...
# 🚨 إعدادات قاعدة البيانات - مستقرة تماماً وتتحمل 100 لاعب بدون انهيار للذاكرة
...
# 🚨 كاشف الأخطاء (يمنع Render من قتل السيرفر)
...
def home():
    ...
    if user and getattr(user, 'role', '') == 'admin' and request.args.get('test_victory'):
        ...
        if tid.isdigit(): test_winner = User.objects(hunter_id=int(tid)).first()
    ...
    if settings and getattr(settings, 'floor3_mode_active', False):
        hunters_list = [{'id': h.hunter_id, 'name': h.username} for h in User.objects(status='active', role='hunter', hunter_id__ne=1000)]
...
def update_settings():
    ...
    if action == 'change_avatar':
        ...
        if last_pw_change and (now - last_pw_change).days < 1: flash('يُسمح بتغيير كلمة السر مرة واحدة كل 24 ساعة!', 'error')
            ...
            flash('تم التغيير بنجاح!', 'success')
    try: user.save()
    except: pass
...
def admin_panel():
    ...
    search_query = request.args.get('search_user', '').strip() # 🚀 ميزة البحث الجديدة للأدمن
    ...
    if request.method == 'POST':
        ...
        try:
            if act == 'moderate_dec':
                ...
                if dur > 0: 
                    settings.maintenance_mode = True; settings.maintenance_until = datetime.utcnow() + timedelta(minutes=dur)
                    settings.maintenance_pages = request.form.getlist('m_pages')
                ...
                settings.gates_description = request.form.get('desc', ''); settings.gate_1_name = request.form.get('g1', '')
                settings.gate_2_name = request.form.get('g2', ''); settings.gate_3_name = request.form.get('g3', '')
                ...
                settings.floor3_mode_active = False; log_action(f"صعد أعلى {top_n} لاعبين للمعركة الأخيرة!", "system", True)
    ...
    # 🚀 فلترة اللاعبين في الإدارة لتسريع البحث
    ...
    # نجلب أحدث 100 فقط لمنع انهيار الذاكرة أثناء العرض
    ...
    return render_template('admin.html', users=users, settings=settings, logs=logs, current_date=sel_date, test_users=User.objects(gate_status='testing', status='active'), gate_stats=gate_stats, floor3_leaders=floor3_leaders, search_query=search_query)

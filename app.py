...
def puzzles():
    try:
        ...
        if request.method == 'POST':
            ...
            # معالجة أنواع الألغاز المختلفة
            if puzzle.puzzle_type == 'text' or puzzle.puzzle_type == 'click_count' or puzzle.puzzle_type == 'word_order':
                guess = request.form.get('guess', '').strip()
                if puzzle.puzzle_type == 'click_count':
                    # لغز الضغط المتكرر: الإجابة هي عدد مرات الضغط المطلوب (يتم تحقيقه من خلال واجهة خاصة)
                    # هنا نفترض أن المستخدم يضغط زراً يزيد العداد، وسيتم إرسال القيمة النهائية كـ "guess"
                    if str(guess) == str(puzzle.puzzle_answer):
                        pass  # سيكمل التحقق أدناه
                    else:
                        flash('عدد الضغطات غير صحيح.', 'error')
                        return redirect(url_for('puzzles'))
                elif puzzle.puzzle_type == 'word_order':
                    # لغز ترتيب الكلمات: الإجابة هي الكلمات بترتيبها الصحيح مفصولة بفواصل
                    if str(guess) == str(puzzle.puzzle_answer):
                        pass
                    else:
...
def secret_link(puzzle_id):
    try:
        ...
        if puzzle.puzzle_type == 'quicksand_trap':
            ...
            item_name = puzzle.puzzle_answer  # اسم العنصر الذي يعطيه
            if str(user.id) not in puzzle.winners_list and puzzle.current_winners < puzzle.max_winners:
                user.inventory.append(item_name)
                puzzle.current_winners += 1
                puzzle.winners_list.append(str(user.id))
            ...
            # يعطي ختم (يضاف إلى الحقيبة كعنصر من نوع seal)
            if str(user.id) not in puzzle.winners_list and puzzle.current_winners < puzzle.max_winners:
            ...
            try:
                if act == 'setup_maintenance':
                    ...
                    if puzzle_type in ['points_gift', 'points_penalty', 'freeze_trap', 'item_reward', 'seal_reward']:
                        ...
                        # الأنواع القديمة (quicksand_trap, hidden_link)
                        News(
                            title="لغز مخفي",
                            content="خفي",
                            category='hidden',

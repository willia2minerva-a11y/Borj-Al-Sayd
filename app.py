...
# حفظ الجلسة لمدة عام لضمان عدم الطرد
...
def pre_process():
    ...
    if settings:
        ...
        # 5. الصيانة
        if getattr(settings, 'maintenance_mode', False) and settings.maintenance_until and now > settings.maintenance_until:
...
def hunter_profile(target_id):
    # حماية ملف الإمبراطور (ID 1000) والحساب المساعد (1001)
...
def admin_panel():
    ...
    # حساب إحصائيات البوابات شاملة من لم يختر

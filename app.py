...
# 🗺️ خريطة المتاهة المعمارية للطابق الأول (نظام التنقل المتصل)
F1_MAP = {
...
# 📜 المهام المتاحة (يوزع عشوائياً 3 لكل لاعب)
TASKS_POOL = [
...
def process_f1_meeting_end(settings, gid_str):
    ...
    if members:
        ...
        for m in members:
            if m.f1_votes_received > max_votes: 
                max_votes = m.f1_votes_received; kicked_user = m; tie = False
        ...
        if kicked_user and not tie:
            ...
            if getattr(kicked_user, 'is_cursed', False):
                GroupMessage(group_id=gid, sender_name="النظام", message="✨ لقد طردتم الملعون! فازت المجموعة!", is_system_msg=True).save()
    ...
    if gid_str in meetings: del meetings[gid_str]
...
def execute_trap_effect(user, trap):
    ...
    if eff == 'give_points':
        ...
        if trap.reward_item: user.update(push__inventory=trap.reward_item)
        ...
        else: flash('نجوت! حاول الفخ سرقتك لكن حقيبتك فارغة.', 'info')
    elif eff == 'give_seal':
        ...
        else: flash('حصلت على ختم تملكه مسبقاً!', 'info')
    elif eff == 'quicksand':
...
def pre_process():
    ...
    except Exception as e: return f"<div style='direction:rtl; background:#0a0a0a; color:#e74c3c; padding:30px; text-align:center;'><h1>🚨 مشكلة اتصال</h1><p>{str(e)}</p></div>", 503
    ...
    if settings:
        ...
        if settings.floor3_mode_active and not getattr(settings, 'floor3_paused', False) and settings.vote_end_time and now >= settings.vote_end_time:
            ...
            slackers_count = slackers.count(); active_count = all_active.count()
            
            if slackers_count > 0 and active_count > 0:
                bonus_votes = (slackers_count * 100.0) / active_count
                for v in all_active: v.update(inc__survival_votes=bonus_votes)
                
            slackers.update(set__status='eliminated', set__freeze_reason='تخاذل في المحكمة')
            for u in User.objects(status='active', role='hunter', hunter_id__ne=1000).order_by('-survival_votes')[:settings.vote_top_n]: u.update(set__zone='المعركة الأخيرة')
            GlobalSettings.objects(setting_name='main_config').update_one(set__floor3_mode_active=False, set__floor3_results_active=True); _settings_cache['timestamp'] = 0

        # 🕵️ انتهاء اجتماعات المجموعات المستقلة
    ...
    if 'user_id' in session:
        ...
        if user.zone in ['الطابق 3', 'المعركة الأخيرة']:
            if getattr(user, 'totem_self', False): user.update(set__totem_self=False); flash('🔥 احترق توتم إعادة الحياة بفعل قوى الطابق!', 'error')
            if getattr(user, 'has_shield', False): user.update(set__has_shield=False); flash('🛡️ احترق وشاح الحماية بفعل قوى الطابق!', 'error')

        if user.status == 'active' and user.quicksand_lock_until and now < user.quicksand_lock_until:
            tl = user.quicksand_lock_until - now; return render_template('locked.html', message=f'مقيّد في الرمال لـ {tl.seconds // 60}د')
...
def get_avatar(hunter_id):
    try:
        ...
        if user:
            if getattr(user, 'status', '') in ['eliminated', 'dead_body']: return Response('''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect width="100" height="100" fill="#110d09"/><text x="50" y="55" font-size="60" text-anchor="middle" dominant-baseline="middle">💀</text></svg>''', mimetype="image/svg+xml", headers={'Cache-Control': 'public, max-age=31536000'})
            if getattr(user, 'status', '') == 'frozen': return Response('''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect width="100" height="100" fill="#110d09"/><text x="50" y="55" font-size="60" text-anchor="middle" dominant-baseline="middle">🧊</text></svg>''', mimetype="image/svg+xml", headers={'Cache-Control': 'public, max-age=31536000'})
...
def home():
    ...
    room_players = []; dead_bodies_in_room = []; is_dark = False; adjacent_rooms = []
    has_meeting = False; meeting_info = None; admin_dots = {}; camera_feeds = {}
    ...
    if settings and getattr(settings, 'floor1_mode_active', False) and user and user.status == 'active':
        ...
        if gid_str in getattr(settings, 'f1_active_meetings', {}):
            has_meeting = True; meeting_info = settings.f1_active_meetings[gid_str]
    ...
    return render_template('index.html', emperor=emperor, test_winner=test_winner, active_hunters=active_hunters, room_players=room_players, dead_bodies_in_room=dead_bodies_in_room, is_dark=is_dark, adjacent_rooms=adjacent_rooms, has_meeting=has_meeting, meeting_info=meeting_info, admin_dots=admin_dots, camera_feeds=camera_feeds, f3_users=User.objects(status='active', role='hunter', survival_votes__gt=0).order_by('-survival_votes'))
...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(getattr(user, 'password_hash', ''), request.form['password']):
            ...
            if user.status in ['eliminated', 'frozen', 'dead_body']: flash('حسابك موقوف.', 'error')
...
def register():
    if request.method == 'POST':
        ...
        flash('تم التسجيل! حسابك قيد المراجعة.', 'success'); return redirect(url_for('login'))
    return render_template('register.html')

@app.route('/logout')
def logout(): session.clear(); return redirect(url_for('home'))

# ⚖️ التصويت الصارم في المحكمة (الطابق 3)
@app.route('/submit_floor3_votes', methods=['POST'])
@login_required
def submit_floor3_votes():
    user = g.user; settings = g.settings
    if getattr(user, 'hunter_id', 0) == 1000 or getattr(user, 'has_voted', False) or getattr(user, 'status', '') != 'active' or getattr(settings, 'floor3_paused', False): return redirect(url_for('home'))
    try:
        tids = []; amts = []
        for i in range(1, 6):
            t_val = request.form.get(f'target_{i}'); a_val = request.form.get(f'amount_{i}')
            if not t_val or not a_val: flash(f'يجب ملء جميع الخانات! الخانة {i} فارغة.', 'error'); return redirect(url_for('home'))
            try:
                t = int(t_val); a = float(a_val)
                if not a.is_integer(): flash('لا ينبغي أن يكون التصويت كسراً أو عدداً عشرياً!', 'error'); return redirect(url_for('home'))
                a = int(a)
            except ValueError: flash('القيم المدخلة غير صالحة!', 'error'); return redirect(url_for('home'))
            if a < 1: flash('لا يمكنك ترك تصويت بـ 0. أعط صوتاً واحداً على الأقل في كل خانة!', 'error'); return redirect(url_for('home'))
            tids.append(t); amts.append(a)

        if sum(amts) != 100: flash(f'مجموع الأصوات يجب أن يكون 100 بالضبط! المجموع الحالي: {sum(amts)}', 'error'); return redirect(url_for('home'))
        if len(set(tids)) != 5: flash('لا يمكن التصويت لنفس الرحالة أكثر من مرة!', 'error'); return redirect(url_for('home'))
        if user.hunter_id in tids: flash('لا يمكنك التصويت لنفسك!', 'error'); return redirect(url_for('home'))
        if 1000 in tids: flash('لا يمكنك التصويت للإمبراطور!', 'error'); return redirect(url_for('home'))

        votes_cast = {}
        for i, tid in enumerate(tids):
            target_user = User.objects(hunter_id=tid, status='active').first()
            if target_user: 
                target_user.update(inc__survival_votes=amts[i])
                votes_cast[str(tid)] = {"name": target_user.username, "amount": amts[i]}
        
        user.update(set__has_voted=True, set__f3_votes_cast=votes_cast); flash('تم تثبيت أصواتك بنجاح!', 'success')
    except Exception as e: flash('حدث خطأ أثناء التصويت.', 'error')
    return redirect(url_for('home'))

# 🕵️ أحداث الطابق الأول (Among Us)
@app.route('/f1_move', methods=['POST'])
@login_required
def f1_move():
    user = g.user; settings = g.settings
    if not settings or not getattr(settings, 'floor1_mode_active', False) or user.status != 'active': return redirect(url_for('home'))
    new_room = request.form.get('room')
    if new_room not in F1_MAP.get(getattr(user, 'current_room', 'الساحة'), []): flash('منطقة بعيدة! لا يمكنك الانتقال إلا للغرف المجاورة.', 'error'); return redirect(url_for('home'))
    cooldown_mins = getattr(settings, 'floor1_move_cooldown', 30); now = datetime.utcnow()
    if user.last_move_time and (now - user.last_move_time).total_seconds() < (cooldown_mins * 60): flash(f'⏳ انتظر {int((cooldown_mins*60) - (now - user.last_move_time).total_seconds())//60} دقيقة للتحرك مجدداً.', 'error')
    else: user.update(set__current_room=new_room, set__last_move_time=now); flash(f'🚶 وصلت إلى {new_room}.', 'success')
    return redirect(url_for('home'))
...
def f1_vent():
    user = g.user; settings = g.settings
    if not settings or not getattr(settings, 'floor1_mode_active', False) or not getattr(user, 'is_cursed', False) or user.status != 'active': return redirect(url_for('home'))
    if getattr(user, 'used_vent', False): flash('لقد استخدمت النفق السري مسبقاً! لقد انهار.', 'error'); return redirect(url_for('home'))
    new_room = request.form.get('target_room')
    user.update(set__current_room=new_room, set__used_vent=True, set__last_move_time=datetime.utcnow()); flash(f'زحفت في الظلام وظهرت في {new_room}.', 'success')
    return redirect(url_for('home'))

@app.route('/f1_do_task', methods=['POST'])
@login_required
def f1_do_task():
    user = g.user; settings = g.settings
    if not settings or not getattr(settings, 'floor1_mode_active', False) or user.status != 'active' or getattr(user, 'is_cursed', False): return redirect(url_for('home'))
    task_name = request.form.get('task_name')
    tasks = list(getattr(user, 'f1_tasks', []))
    new_tasks = [t for t in tasks if t.get('name') != task_name]
    
    if len(tasks) != len(new_tasks):
        user.update(set__f1_tasks=new_tasks, inc__gems_collected=1); flash('تم إنجاز المهمة بنجاح!', 'success')
        ...
        if total_gems >= getattr(settings, 'floor1_gems_target', 10):
            ...
            GroupMessage(group_id=user.group_id, sender_name="النظام", message="✨ اكتملت المهام! احترقت اللعنة وفُتحت البوابة!", is_system_msg=True).save()
...
def f1_report(target_id):
    ...
    if body:
        meeting_duration = 5 
        meetings = settings.f1_active_meetings
        meetings[str(user.group_id)] = {
            'end_time': (datetime.utcnow() + timedelta(minutes=meeting_duration)).isoformat(),
            'caller': user.username, 'type': 'report', 'body': body.username
        }
        GlobalSettings.objects(setting_name='main_config').update_one(set__f1_active_meetings=meetings)
        GroupMessage(group_id=user.group_id, sender_name="النظام", message=f"🚨 {user.username} بلّغ عن جثة {body.username}!", is_system_msg=True).save()
        flash('تم إطلاق الإنذار!', 'success')
    return redirect(url_for('home'))
...
def f1_emergency():
    ...
    if getattr(user, 'emergency_used', False): flash('استهلكت زر الطوارئ!', 'error')
        ...
        meetings[str(user.group_id)] = {
            'end_time': (datetime.utcnow() + timedelta(minutes=meeting_duration)).isoformat(),
            'caller': user.username, 'type': 'emergency', 'body': ''
        }
        GlobalSettings.objects(setting_name='main_config').update_one(set__f1_active_meetings=meetings)
        user.update(set__emergency_used=True)
        GroupMessage(group_id=user.group_id, sender_name="النظام", message=f"🚨 قام {user.username} باستدعاء طارئ!", is_system_msg=True).save()
        flash('تم استدعاء المجموعة!', 'success')
    return redirect(url_for('home'))
...
def f1_kill(target_id):
    ...
    if not settings or not getattr(settings, 'floor1_mode_active', False) or not getattr(attacker, 'is_cursed', False) or attacker.status != 'active': return redirect(url_for('home'))
    ...
    if attacker.last_kill_time and (now - attacker.last_kill_time).total_seconds() < (cooldown_mins * 60): flash('🔪 اللعنة لم تسترجع قوتها!', 'error')
        ...
        target.update(set__status='dead_body', set__freeze_reason='طُعن في الظلام'); attacker.update(set__last_kill_time=now); flash('🩸 تمت التضحية!', 'success')
        
        # 🔔 إرسال إشعار للشهود في الغرفة والكاميرات
        for w in User.objects(group_id=attacker.group_id, status='active', current_room=attacker.current_room, id__ne=attacker.id):
            Notification(target_hunter_id=w.hunter_id, message=f'⚠️ لمحت خيالاً يسقط! تم اغتيال {target.username} أمامك!', notif_type='danger').save()
        if attacker.current_room in ['الممر العظيم', 'الساحة', 'قاعة المخطوطات']:
            for watcher in User.objects(group_id=attacker.group_id, status='active', current_room='مرصد النجوم'):
                Notification(target_hunter_id=watcher.hunter_id, message=f'📷 عبر مرصد النجوم: رأيت {attacker.username} يغتال {target.username} في {attacker.current_room}!', notif_type='danger').save()
...
def meeting_room():
    ...
    meeting_info = settings.f1_active_meetings.get(str(user.group_id)) if settings else None
    return render_template('meeting.html', user=user, settings=settings, messages=messages, group_members=group_members, meeting_info=meeting_info)

@app.route('/f1_chat', methods=['POST'])
@login_required
def f1_chat():
    user = g.user; settings = g.settings
    if getattr(settings, 'floor1_mode_active', False) and user.status == 'active' and str(user.group_id) in getattr(settings, 'f1_active_meetings', {}):
...
def f1_vote(target_id):
    user = g.user; settings = g.settings
    if not settings or str(user.group_id) not in getattr(settings, 'f1_active_meetings', {}) or user.status != 'active' or getattr(user, 'f1_has_voted', False): return redirect(url_for('meeting_room'))
    
    if target_id == 0:
        user.update(set__f1_has_voted=True)
        GroupMessage(group_id=user.group_id, sender_name="النظام", message=f"👁️ قام {user.username} بتخطي التصويت.", is_system_msg=True).save(); flash('تم تخطي التصويت!', 'info')
    else:
        ...
        if target:
            ...
            GroupMessage(group_id=user.group_id, sender_name="النظام", message=f"👁️ قام {user.username} بالتصويت ضد {target.username}.", is_system_msg=True).save(); flash('تم تسجيل تصويتك العلني!', 'success')
    return redirect(url_for('meeting_room'))
...
def inject_globals():
    ...
    if notifs['current_user']:
        u = notifs['current_user']; now = datetime.utcnow()
        try:
            notifs['un_news'] = News.objects(category='news', status='approved', created_at__gt=(u.last_seen_news or now)).count()
            notifs['un_puz'] = News.objects(category='puzzle', status='approved', created_at__gt=(u.last_seen_puzzles or now)).count()
            notifs['un_dec'] = News.objects(category='declaration', status='approved', created_at__gt=(u.last_seen_decs or now)).count()
            notifs['un_store'] = StoreItem.objects(created_at__gt=(u.last_seen_store or now)).count()
            notifs['unread_notifs'] = Notification.objects(target_hunter_id=u.hunter_id, is_read=False).count()
        except: pass
    return {**notifs, 'default_avatar': "data:image/svg+xml;base64," + base64.b64encode(b'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect width="100" height="100" fill="#110d09"/><circle cx="50" cy="35" r="20" fill="#d4af37"/><path d="M20 90c0-20 15-35 30-35s30 15 30 35" fill="#d4af37"/></svg>').decode('utf-8')}
...
def admin_required(f):
    @wraps(f)
    def decorated(*args, **kwargs):
        if not g.user or g.user.role != 'admin': return redirect(url_for('home'))
        return f(*args, **kwargs)
    return decorated
...
def f1_do_task():
    user = g.user; settings = g.settings
    if not settings or not getattr(settings, 'floor1_mode_active', False) or user.status != 'active' or getattr(user, 'is_cursed', False): return redirect(url_for('home'))
    task_name = request.form.get('task_name'); tasks = list(getattr(user, 'f1_tasks', [])); new_tasks = [t for t in tasks if t.get('name') != task_name]
    if len(tasks) != len(new_tasks):
        user.update(set__f1_tasks=new_tasks, inc__gems_collected=1); flash('تم إنجاز المهمة بنجاح!', 'success')
        total_gems = sum([u.gems_collected for u in User.objects(group_id=user.group_id)])
        if total_gems >= getattr(settings, 'floor1_gems_target', 10):
...
def f1_report(target_id):
    ...
    if body:
        meeting_duration = 5 
        meetings = settings.f1_active_meetings
        meetings[str(user.group_id)] = {'end_time': (datetime.utcnow() + timedelta(minutes=meeting_duration)).isoformat(), 'caller': user.username, 'type': 'report', 'body': body.username}
        GlobalSettings.objects(setting_name='main_config').update_one(set__f1_active_meetings=meetings)
        GroupMessage(group_id=user.group_id, sender_name="النظام", message=f"🚨 {user.username} بلّغ عن جثة {body.username}!", is_system_msg=True).save()
        flash('تم إطلاق الإنذار!', 'success')
    return redirect(url_for('home'))
...
def f1_emergency():
    ...
    if getattr(user, 'emergency_used', False): flash('استهلكت زر الطوارئ!', 'error')
        ...
        meeting_duration = 5; meetings = settings.f1_active_meetings
        meetings[str(user.group_id)] = {'end_time': (datetime.utcnow() + timedelta(minutes=meeting_duration)).isoformat(), 'caller': user.username, 'type': 'emergency', 'body': ''}
        GlobalSettings.objects(setting_name='main_config').update_one(set__f1_active_meetings=meetings); user.update(set__emergency_used=True)
        GroupMessage(group_id=user.group_id, sender_name="النظام", message=f"🚨 قام {user.username} باستدعاء طارئ!", is_system_msg=True).save()
        flash('تم استدعاء المجموعة!', 'success')
    return redirect(url_for('home'))
...
def f1_vote(target_id):
    ...
    if not settings or str(user.group_id) not in getattr(settings, 'f1_active_meetings', {}) or user.status != 'active' or getattr(user, 'f1_has_voted', False): return redirect(url_for('meeting_room'))
    if target_id == 0:
        user.update(set__f1_has_voted=True); GroupMessage(group_id=user.group_id, sender_name="النظام", message=f"👁️ قام {user.username} بتخطي التصويت.", is_system_msg=True).save(); flash('تم تخطي التصويت!', 'info')
    else:
        target = User.objects(hunter_id=target_id, status='active', group_id=user.group_id).first()
        if target:
            target.update(inc__f1_votes_received=1); user.update(set__f1_has_voted=True); GroupMessage(group_id=user.group_id, sender_name="النظام", message=f"👁️ قام {user.username} بالتصويت ضد {target.username}.", is_system_msg=True).save(); flash('تم تسجيل تصويتك العلني!', 'success')
    return redirect(url_for('meeting_room'))
...
def admin_panel():
    ...
    if request.method == 'POST':
        ...
        if act == 'toggle_gates':
            if getattr(settings, 'gates_mode_active', False): GlobalSettings.objects(setting_name='main_config').update_one(set__gates_mode_active=False); flash('تم إيقاف البوابات.', 'success')
            else:
                ...
                GlobalSettings.objects(setting_name='main_config').update_one(set__gates_mode_active=True, set__gates_end_time=g_end, set__gates_description=request.form.get('desc', ''), set__gate_1_name=request.form.get('g1', ''), set__gate_2_name=request.form.get('g2', ''), set__gate_3_name=request.form.get('g3', '')); flash('تم فتح البوابات!', 'success')
        elif act == 'toggle_floor1':
            if getattr(settings, 'floor1_mode_active', False): GlobalSettings.objects(setting_name='main_config').update_one(set__floor1_mode_active=False, set__f1_active_meetings={}); flash('تم إيقاف الطابق الأول.', 'success')
            else:
                active_users = list(User.objects(status='active', hunter_id__ne=1000)); random.shuffle(active_users); group_size = int(request.form.get('group_size', 5)); group_id = 1
                for i in range(0, len(active_users), group_size):
                    ...
                    if members:
                        ...
                        for j, m in enumerate(members): 
                            ...
                            m.update(set__group_id=group_id, set__is_cursed=(j==cursed_idx), set__current_room='الساحة', set__f1_has_voted=False, set__f1_votes_received=0, set__f1_tasks=random_tasks)
                        group_id += 1
                GlobalSettings.objects(setting_name='main_config').update_one(set__floor1_mode_active=True, set__floor1_move_cooldown=int(request.form.get('move_cooldown', 30)), set__f1_active_meetings={}); flash('بدأ الطابق الأول ووزعت اللعنات والمهام!', 'success')
        elif act == 'cancel_f1_meeting':
            gid = request.form.get('group_id')
            if gid and str(gid) in settings.f1_active_meetings:
                meetings = settings.f1_active_meetings; del meetings[str(gid)]
                GlobalSettings.objects(setting_name='main_config').update_one(set__f1_active_meetings=meetings); User.objects(group_id=int(gid)).update(set__f1_has_voted=False, set__f1_votes_received=0); flash(f'تم إلغاء اجتماع المجموعة {gid}!', 'success')
            else: flash('هذه المجموعة ليست في اجتماع!', 'error')
        elif act == 'toggle_war':
            new_state = not getattr(settings, 'war_mode', False); war_hours = int(request.form.get('war_hours') or 0); end_time = datetime.utcnow() + timedelta(hours=war_hours) if war_hours > 0 and new_state else None
            ...
            if getattr(settings, 'floor3_mode_active', False):
                ...
                GlobalSettings.objects(setting_name='main_config').update_one(set__floor3_mode_active=False, set__floor3_paused=True, set__floor3_time_left=max(0, int(time_left))); flash('تم تجميد المحكمة وحفظ الوقت!', 'success')
            elif getattr(settings, 'floor3_paused', False):
                new_end = datetime.utcnow() + timedelta(seconds=settings.floor3_time_left)
                GlobalSettings.objects(setting_name='main_config').update_one(set__floor3_mode_active=True, set__floor3_paused=False, set__vote_end_time=new_end); flash('تم استئناف المحكمة!', 'success')
            else:
                vote_hours = int(request.form.get('vote_hours') or 0)
                GlobalSettings.objects(setting_name='main_config').update_one(set__floor3_mode_active=True, set__floor3_results_active=False, set__vote_end_time=datetime.utcnow() + timedelta(hours=vote_hours) if vote_hours > 0 else None, set__vote_top_n=int(request.form.get('top_n', 5))); flash('بدأت المحكمة الكبرى!', 'success')
        elif act == 'reset_floor3':
            User.objects(role='hunter').update(set__has_voted=False, set__survival_votes=0.0, set__f3_votes_cast={})
            GlobalSettings.objects(setting_name='main_config').update_one(set__floor3_mode_active=False, set__floor3_paused=False, set__floor3_results_active=False); flash('تم تصفير المحكمة بالكامل!', 'success')
        elif act == 'toggle_final_battle': 
            new_state = not getattr(settings, 'final_battle_mode', False)
            if new_state: emp_hp = int(request.form.get('emperor_hp') or 100000); User.objects(hunter_id=1000).update(set__health=emp_hp)
            GlobalSettings.objects(setting_name='main_config').update_one(set__final_battle_mode=new_state); flash('تغيرت حالة المعركة الأخيرة!', 'success')
            ...
            flash('تم زرع الكيان بنجاح!', 'success')
    ...
    users_query = User.objects(hunter_id__ne=1000); search_query = request.args.get('search_user', '').strip() 
    if search_query: users_query = users_query.filter(hunter_id=int(search_query)) if search_query.isdigit() else users_query.filter(username__icontains=search_query)
...
def add_friend():
    ...
    if getattr(target, 'role', '') in ['ghost', 'cursed_ghost']:
        ...
        if trap and str(user.id) not in getattr(trap, 'winners_list', []) and getattr(trap, 'current_winners', 0) < getattr(trap, 'max_winners', 1):
            if trap.puzzle_type.startswith('ghost_'): execute_trap_effect(user, trap); trap.update(inc__current_winners=1, push__winners_list=str(user.id))
...
def cancel_request():
    user = g.user; target = User.objects(hunter_id=int(request.form.get('target_id') or 0)).first()
    if target and user.hunter_id in getattr(target, 'friend_requests', []): target.update(pull__friend_requests=user.hunter_id); flash('تم السحب.', 'info')
    return redirect(request.referrer or url_for('home'))
...
def transfer(target_id):
    ...
    if transfer_type == 'points':
        ...
        if itm in getattr(sender, 'inventory', []): sender.update(pull__inventory=itm, inc__loyalty_points=5); receiver.update(push__inventory=itm); flash('تم الإرسال!', 'success')
    return redirect(request.referrer or url_for('home'))
...
def puzzles():
    user = g.user; settings = g.settings
    if request.method == 'POST':
        if user.status != 'active': flash('حسابك موقوف.', 'error'); return redirect(url_for('puzzles'))
...
def secret_link(puzzle_id):
    ...
    if puzzle and str(user.id) not in getattr(puzzle, 'winners_list', []) and getattr(puzzle, 'current_winners', 0) < getattr(puzzle, 'max_winners', 1):
        if puzzle.puzzle_type.startswith('link_'): execute_trap_effect(user, puzzle); puzzle.update(inc__current_winners=1, push__winners_list=str(user.id))
    return redirect(request.referrer or url_for('home'))
...
def use_item(target_id):
    ...
    if item_type in ['weapon', 'steal', 'spy']:
        ...
        if cooldown_mins > 0 and attacker.last_action_time and (now - attacker.last_action_time).total_seconds() < (cooldown_mins * 60):
            flash(f'انتظر {int((cooldown_mins*60) - (now - attacker.last_action_time).total_seconds())} ثانية.', 'error'); return redirect(request.referrer or url_for('home'))
    ...
    if item_type == 'weapon':
        ...
        elif getattr(target, 'has_shield', False): target.update(set__has_shield=False); flash('انكسر درع الهدف!', 'error')
        else:
        ...
        if target.id == attacker.id:
            ...
            if item_name not in collected:
                attacker.update(push__collected_seals=item_name, pull__inventory=item_name); attacker.reload()
                ...
                else: flash(f'فُعل الختم!', 'success')
...
def altar():
    ...
    if request.method == 'POST':
        ...
        now = datetime.utcnow()
        if getattr(spell, 'expires_at', None) and now > spell.expires_at: flash('خطأ! تلاشت طاقة هذه التعويذة.', 'error'); return redirect(url_for('altar'))
        used_list = getattr(spell, 'used_by', []); max_u = getattr(spell, 'max_uses', 0)
        if str(user.id) in used_list: flash('خطأ! استخدمت التعويذة مسبقاً.', 'error'); return redirect(url_for('altar'))
        if max_u > 0 and len(used_list) >= max_u: flash('خطأ! استنفدت طاقة التعويذة.', 'error'); return redirect(url_for('altar'))
        try:
            ...
            elif stype == 'points_loss': loss = int(user.points * (val / 100.0)) if is_perc else val; user.update(set__points=max(0, user.points - loss)); flash('خطأ! سحب منك المذبح دنانير.', 'error')
...
@login_required
def poneglyph():
    if getattr(g.user, 'role', '') != 'admin' and not getattr(g.user, 'unlocked_lore_room', False): return redirect(url_for('home'))
    return render_template('poneglyph.html', poneglyph_text=getattr(g.settings, 'poneglyph_text', ''))

@app.route('/top_room')
@login_required
def top_room():
    if getattr(g.user, 'role', '') != 'admin' and not getattr(g.user, 'unlocked_top_room', False): return redirect(url_for('home'))
    return render_template('top_room.html', top_iq=User.objects(hunter_id__ne=1000, status__in=['active', 'inactive']).order_by('-intelligence_points')[:10], top_loyal=User.objects(hunter_id__ne=1000, status__in=['active', 'inactive']).order_by('-loyalty_points')[:10], top_hp=User.objects(hunter_id__ne=1000, status__in=['active', 'inactive']).order_by('-health')[:10])
...
def choose_gate():
    user = g.user; settings = g.settings
    if user.status != 'active': return redirect(url_for('home'))
    if getattr(settings, 'gates_mode_active', False) and getattr(user, 'chosen_gate', 0) == 0: user.update(set__chosen_gate=int(request.form.get('gate_num') or 0), set__gate_status='waiting')
    return redirect(url_for('home'))
...
def admin_update_profile(target_id):
    ...
    if target_user:
        try:
            action = request.form.get('action'); val = request.form.get('new_val')

...
F1_MAP = {
    'الساحة': ['الممر الرئيسي', 'المستودع'],
    'الممر الرئيسي': ['الساحة', 'المكتبة', 'غرفة الكاميرات'],
    'المكتبة': ['الممر الرئيسي', 'مذبح الطلاسم'],
    'المستودع': ['الساحة'],
    'غرفة الكاميرات': ['الممر الرئيسي'],
    'مذبح الطلاسم': ['المكتبة', 'بوابة الطابق 2'],
...
def pre_process():
    ...
    if settings:
        ...
        if getattr(settings, 'floor1_mode_active', False) and getattr(settings, 'f1_active_meetings', {}):
            for gid_str, end_time_str in settings.f1_active_meetings.items():
...
def home():
    ...
    has_meeting = False; meeting_end_time = None
    ...
    if settings and getattr(settings, 'floor1_mode_active', False) and user and user.status == 'active':
        ...
        if gid_str in getattr(settings, 'f1_active_meetings', {}):
            has_meeting = True; meeting_end_time = settings.f1_active_meetings[gid_str]
    ...
    return render_template('index.html', emperor=emperor, test_winner=test_winner, active_hunters=active_hunters, room_players=room_players, dead_bodies_in_room=dead_bodies_in_room, is_dark=is_dark, adjacent_rooms=adjacent_rooms, has_meeting=has_meeting, meeting_end_time=meeting_end_time, f3_users=User.objects(status='active', role='hunter', survival_votes__gt=0).order_by('-survival_votes'))
        ...
        if puzzle and str(guess) == str(getattr(puzzle, 'puzzle_answer', '')) and str(user.id) not in getattr(puzzle, 'winners_list', []):
            if getattr(puzzle, 'current_winners', 0) < getattr(puzzle, 'max_winners', 1):
                ...
                if settings and getattr(settings, 'floor1_mode_active', False):
                    user.update(inc__gems_collected=1); Notification(target_hunter_id=user.hunter_id, message='💎 حصلت على حجر!', notif_type='success').save()
...
def submit_floor3_votes():
    ...
    try:
        ...
        for i in range(1, 6):
            ...
            if a < 1: flash('لا يمكنك ترك تصويت بـ 0. أعط صوتاً واحداً على الأقل!', 'error'); return redirect(url_for('home'))
            tids.append(t); amts.append(a)

        if sum(amts) != 100: flash(f'مجموع الأصوات يجب أن يكون 100! مجموعك: {sum(amts)}', 'error'); return redirect(url_for('home'))
        if len(set(tids)) != 5: flash('لا يمكن التصويت لنفس الرحالة في أكثر من خانة!', 'error'); return redirect(url_for('home'))
        ...
        for i, tid in enumerate(tids):
            ...
            if target_user: target_user.update(inc__survival_votes=amts[i]); votes_cast[str(tid)] = {"name": target_user.username, "amount": amts[i]}
...
def f1_move():
    ...
    if new_room not in F1_MAP.get(getattr(user, 'current_room', 'الساحة'), []): flash('لا يمكنك الوصول لهذه الغرفة مباشرة! انظر الخريطة.', 'error'); return redirect(url_for('home'))
    cooldown_mins = getattr(settings, 'floor1_move_cooldown', 30); now = datetime.utcnow()
    if user.last_move_time and (now - user.last_move_time).total_seconds() < (cooldown_mins * 60): flash(f'⏳ انتظر {int((cooldown_mins*60) - (now - user.last_move_time).total_seconds())//60} دقيقة.', 'error')
...
def f1_report(target_id):
    ...
    if body:
        meeting_duration = 5 
        meetings = settings.f1_active_meetings; meetings[str(user.group_id)] = (datetime.utcnow() + timedelta(minutes=meeting_duration)).isoformat()
        GlobalSettings.objects(setting_name='main_config').update_one(set__f1_active_meetings=meetings)
...
def f1_kill(target_id):
    ...
    if attacker.last_kill_time and (now - attacker.last_kill_time).total_seconds() < (cooldown_mins * 60): flash('🔪 انتظر لترتاح اللعنة!', 'error')
    else:
...
def meeting_room():
    ...
    meeting_end = settings.f1_active_meetings.get(str(user.group_id)) if settings else None
    return render_template('meeting.html', user=user, settings=settings, messages=messages, group_members=group_members, meeting_end_time=meeting_end)
    ...
    if request.method == 'POST':
        ...
        if act == 'toggle_gates':
            ...
            if getattr(settings, 'floor1_mode_active', False):
                GlobalSettings.objects(setting_name='main_config').update_one(set__floor1_mode_active=False, set__f1_active_meetings={}); flash('تم إيقاف الطابق الأول.', 'success')
            else:
                ...
                for i in range(0, len(active_users), group_size):
                    ...
                    if members:
                        ...
                        group_id += 1
                GlobalSettings.objects(setting_name='main_config').update_one(set__floor1_mode_active=True, set__floor1_move_cooldown=int(request.form.get('move_cooldown', 30)), set__f1_active_meetings={}); flash('بدأ الطابق الأول ووزعت اللعنات!', 'success')
            ...
            if getattr(settings, 'floor3_mode_active', False):
                ...
                GlobalSettings.objects(setting_name='main_config').update_one(set__floor3_mode_active=False, set__floor3_paused=True, set__floor3_time_left=max(0, int(time_left)))
                ...
                GlobalSettings.objects(setting_name='main_config').update_one(set__floor3_mode_active=True, set__floor3_paused=False, set__vote_end_time=new_end)
                flash('تم استئناف المحكمة الكبرى!', 'success')
            else:
                vote_hours = int(request.form.get('vote_hours') or 0)
                GlobalSettings.objects(setting_name='main_config').update_one(set__floor3_mode_active=True, set__floor3_results_active=False, set__vote_end_time=datetime.utcnow() + timedelta(hours=vote_hours) if vote_hours > 0 else None, set__vote_top_n=int(request.form.get('top_n', 5)))
            ...
            GlobalSettings.objects(setting_name='main_config').update_one(set__floor3_mode_active=False, set__floor3_paused=False, set__floor3_results_active=False)

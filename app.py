...
TASKS_BY_ROOM = {
    'قاعة العروش': [{"name": "إصلاح العرش", "description": "أصلح العرش الملكي"}, {"name": "تنظيف القاعة", "description": "أزل الغبار"}],
    'مكتبة الأسرار': [{"name": "فك التشفير", "description": "فك رموز المخطوطة"}, {"name": "ترتيب الرفوف", "description": "رتب الكتب"}],
    'معبد الطلاسم': [{"name": "رسم الدائرة", "description": "ارسم دائرة الحماية"}, {"name": "ترتيب التماثيل", "description": "أعد ترتيب التماثيل"}],
    'مخزن الآثار': [{"name": "جرد الآثار", "description": "إحصاء القطع"}, {"name": "تنظيف الصناديق", "description": "نظف الصناديق"}],
    'دهليز الأجداد': [{"name": "إضاءة المشاعل", "description": "أشعل المشاعل"}, {"name": "إصلاح الجدار", "description": "صلح الشقوق"}],
    'مرصد الأبراج': [{"name": "توجيه العدسات", "description": "اضبط العدسات"}, {"name": "تسجيل الأبراج", "description": "دوّن المواقع"}],
    'غرفة البصيرة': [{"name": "تأمل البلورة", "description": "تأمل البلورة"}, {"name": "قراءة الطالع", "description": "اقرأ المستقبل"}]
}
...
def execute_trap_effect(user, trap):
    ...
    if eff == 'give_points':
        ...
        user.update(push__inventory=trap.reward_item)
        flash(f'نجاح أسطوري! عثرت على ختم: {trap.reward_item}', 'success')
    elif eff == 'quicksand':
...
def check_lazy_death_and_bleed(user, settings):
    try:
        ...
        if user.hunter_id in [1000, 1001]: return
        
        now = datetime.utcnow()
        last_act = getattr(user, 'last_active', None) or getattr(user, 'created_at', None) or now
        
        # الموت بسبب الخمول (72 ساعة)
        if (now - last_act).total_seconds() / 3600.0 > 72: 
            user.update(set__health=0, set__status='eliminated', set__freeze_reason='ابتلعته الرمال للكسل')
            return
            
        if getattr(settings, 'war_mode', False) and user.status == 'active' and not getattr(settings, 'sleep_mode_active', False):
            ...
            if now > safe_until:
                ...
                if bleed_rate > 0 and minutes_passed >= bleed_rate:
                    ...
                    if new_health <= 0:
                        user.update(set__health=0, set__status='eliminated', set__freeze_reason='نزف حتى الموت (ط2)', set__last_health_check=now)
...
def pre_process():
    ...
    if settings:
        ...
        if settings.sleep_mode_active:
            ...
            if not user_is_admin and request.endpoint not in ['static', 'login', 'logout']:
                return render_template('locked.html', message='الليل خيّم على سيفار... يمنع التجول واللعب حتى شروق الشمس 🌙')
        
        # --- تحديثات الأنظمة والمؤقتات ---
        # إذا لم يكن الموقع في حالة سبات، نتحقق من انتهاء المؤقتات
        if not settings.sleep_mode_active:
            ...
            if settings.gates_mode_active and settings.gates_end_time and now >= settings.gates_end_time:
                User.objects(status='active', chosen_gate=0, hunter_id__nin=[1000, 1001]).update(set__status='eliminated', set__freeze_reason='انتهى وقت البوابات وتخاذل')
    ...
    if 'user_id' in session:
        ...
        if user.status == 'active' and user.quicksand_lock_until and now < user.quicksand_lock_until:
            return render_template('locked.html', message=f'مقيّد في الرمال لـ {(user.quicksand_lock_until - now).seconds // 60}د')
        if user.gate_status == 'testing' and request.endpoint not in ['submit_gate_test', 'logout']: 
            return render_template('gate_test.html', message=getattr(settings, 'gates_test_question', 'ما هو سر المتاهة؟'), user=user)
...
def home():
    ...
    room_players, dead_bodies_in_room, adjacent_rooms = [], [], []
    ...
    if settings and getattr(settings, 'floor1_mode_active', False) and user and user.status == 'active' and user.group_id > 0:
        ...
        if not is_dark: room_players = User.objects(group_id=user.group_id, current_room=user.current_room, status='active', id__ne=user.id, hunter_id__nin=[1000, 1001])
    ...
    return render_template('index.html', 
        winner_user=winner_user, emperor=emperor, test_winner=test_winner, active_hunters=active_hunters, 
        room_players=room_players, dead_bodies_in_room=dead_bodies_in_room, 
        is_dark=is_dark, adjacent_rooms=adjacent_rooms, has_meeting=has_meeting, 
        meeting_info=meeting_info, 
        f3_users=f3_users, f1_map=F1_MAP, player_tasks=player_tasks, current_room_tasks=current_room_tasks, settings=settings
    )

# ==========================================
# 🛡️ نظام الدخول والتسجيل الذكي
# ==========================================
...
def register():
    if request.method == 'POST':
        ...
        # خوارزمية التسجيل الذكية (أعلى ID متواجد + 1)
        last_user = User.objects().order_by('-hunter_id').first()
        if last_user and last_user.hunter_id >= 1001:
            new_id = last_user.hunter_id + 1
        else:
            new_id = 1002
        ...
        flash(f'تم التسجيل بنجاح! رقم الـ ID الخاص بك هو: {new_id}', 'success')
        return redirect(url_for('login'))
...
def profile():
    ...
    return render_template('profile.html', user=user, my_items=my_items, my_seals=[i for i in my_items if getattr(i, 'item_type', '') == 'seal'], settings=fresh_settings)
    ...
    try:
        if action == 'change_avatar':
            ...
            if getattr(user, 'last_name_change', None) and (now - user.last_name_change).days < 7: 
                flash('تغيير الاسم متاح مرة كل 7 أيام!', 'error')
...
@app.route('/shatter_seal', methods=['POST'])
@login_required
def shatter_seal():
    user = g.user
    seal_name = request.form.get('seal_name')
    
    if seal_name in user.inventory and seal_name not in user.collected_seals:
        user.update(pull__inventory=seal_name, push__collected_seals=seal_name)
        user.reload()
        
        # الفوز عبر الأختام الأربعة المتباينة
        if len(set(user.collected_seals)) >= 4:
            GlobalSettings.objects(setting_name='main_config').update_one(set__war_mode=False, set__final_battle_mode=False, set__maze_winner_id=user.hunter_id)
            User.objects(status='active', hunter_id__nin=[1000, 1001]).update(set__health=100)
            flash('🔥 لقد حطمت الأختام الأربعة وفتحت بوابة الحقيقة! لقد قهرت المتاهة!', 'success')
        else:
            flash(f'✨ تم تحطيم [{seal_name}]! طاقته تسري في عروقك.', 'success')
    return redirect(request.referrer or url_for('profile'))

# ==========================================
# 📜 المراسيم والنقوش والألغاز
# ==========================================
@app.route('/news')
@login_required
def news(): 
...
def declarations():
    ...
    if request.method == 'POST':
        ...
        if user.role == 'admin': flash('تم نشر الإرادة الإمبراطورية في الساحة!', 'success')
        else: flash('تم رفع تصريحك للإمبراطور للبت فيه.', 'success')
        return redirect(url_for('declarations'))
...
@app.route('/graveyard')
def graveyard(): 
    return render_template('graveyard.html', users=User.objects(status='eliminated', hunter_id__nin=[1000, 1001]).order_by('-id'))
        ...
        try:
            ...
            if stype == 'hp_loss':
                ...
                if getattr(settings, 'final_battle_mode', False): 
                    ...
                    GlobalSettings.objects(setting_name='main_config').update_one(set__final_battle_mode=False, set__war_mode=False, set__maze_winner_id=user.hunter_id)
...
# ==========================================
# 👑 لوحة الإدارة الشاملة (Admin Panel)
# ==========================================
...
def admin_panel():
    ...
    if request.method == 'POST':
        ...
        # --- 1. إدارة السبات الزمني (Time Shifting) ---
        if act == 'toggle_sleep':
            if settings.sleep_mode_active:
                ...
                # إزاحة كل المؤقتات لتعويض فترة النوم
                ...
                if settings.last_global_bleed: updates['set__last_global_bleed'] = settings.last_global_bleed + duration
                
                GlobalSettings.objects(setting_name='main_config').update_one(**updates)
                flash(f'تم إيقاظ المتاهة! الزمن تم إزاحته بمقدار {duration.total_seconds() // 3600} ساعات و {(duration.total_seconds() % 3600) // 60} دقيقة لضمان العدل.', 'success')
            else:
                GlobalSettings.objects(setting_name='main_config').update_one(set__sleep_mode_active=True, set__sleep_start_time=now)
                flash('المتاهة الآن في سبات عميق. لقد تجمد الزمن وتوقف اللعب.', 'success')

        # --- 2. إدارة أدوات السوق المظلم ---
            ...
            StoreItem(
                ...
                effect_amount=int(request.form.get('effect_amount', 0)),
                is_mirage=bool(request.form.get('is_mirage')),
                mirage_message=request.form.get('mirage_msg', ''),
                is_luck=bool(request.form.get('is_luck')),
                luck_min=int(request.form.get('luck_min', 0)),
                luck_max=int(request.form.get('luck_max', 0))
            ).save()
            flash('تم زرع الأداة في السوق المظلم بنجاح!', 'success')

        # (بقية أوامر الأدمن السابقة تم دمجها هنا بدون نقص...)
            ...
            if getattr(settings, 'gates_mode_active', False):
                ...
                g_end = now + timedelta(hours=gates_hours) if gates_hours > 0 else None
                User.objects(status='active', hunter_id__nin=[1000, 1001]).update(set__chosen_gate=0, set__gate_status='')
                GlobalSettings.objects(setting_name='main_config').update_one(
                    ...
                    set__gates_test_question=request.form.get('test_q', 'ما هو سر المتاهة؟'),
                    set__gates_test_answer=request.form.get('test_a', 'سيفار')
                )
                flash('تم فتح البوابات وتمسير اختيارات الرحالة السابقة للبدء من جديد!', 'success')
                ...
                for i in range(0, len(active_users), group_size):
                    ...
                    if members:
                        ...
                        for j, m in enumerate(members): 
                            ...
                            m.update(
                                set__group_id=group_id, set__is_cursed=(j == cursed_idx), set__current_room='قاعة العروش', 
                                set__f1_has_voted=False, set__f1_votes_received=0, set__gems_collected=0, 
                                set__used_vent=False, set__emergency_used=False, set__f1_last_move=None
                            )
                        group_id += 1
                GlobalSettings.objects(setting_name='main_config').update_one(
                    set__floor1_mode_active=True, set__floor1_move_cooldown=int(request.form.get('move_cooldown', 30)), 
                    set__floor1_gems_target=int(request.form.get('gems_target', 10)), set__f1_active_meetings={}
                )
            ...
            end_time = now + timedelta(hours=war_hours) if war_hours > 0 and new_state else None
            GlobalSettings.objects(setting_name='main_config').update_one(
                set__war_mode=new_state, set__war_end_time=end_time,
                ...
                set__last_global_bleed=now if new_state else settings.last_global_bleed
            )
            if not new_state: User.objects(status='active', hunter_id__nin=[1000, 1001]).update(set__health=100)
            flash('تغيرت إعدادات الحرب الشاملة!', 'success')
            
        elif act == 'toggle_floor3':
            if getattr(settings, 'floor3_mode_active', False):
                time_left = (settings.vote_end_time - now).total_seconds() if settings.vote_end_time else 0
                ...
                flash('تم تجميد المحكمة وحفظ الوقت!', 'success')
            elif getattr(settings, 'floor3_paused', False):
                new_end = now + timedelta(seconds=settings.floor3_time_left)
                ...
                GlobalSettings.objects(setting_name='main_config').update_one(
                    ...
                    set__vote_end_time=now + timedelta(hours=vote_hours) if vote_hours > 0 else None, 
            ...
            News(
                ...
                puzzle_type=ptype, puzzle_answer=request.form.get('puzzle_answer', ''),
                reward_points=int(request.form.get('reward_points') or 0), max_winners=int(request.form.get('max_winners') or 1),
                author='الإمبراطور', status='approved'
            ).save()
            flash('تم إصدار المرسوم/اللغز بنجاح!', 'success')

        elif act == 'add_spell':
            SpellConfig(
                spell_word=request.form.get('spell_word', '').strip(), spell_type=request.form.get('spell_type', ''),
                effect_value=int(request.form.get('effect_value') or 0), item_name=request.form.get('item_name', ''),
                max_uses=int(request.form.get('max_uses') or 1),
                expires_at=now + timedelta(hours=int(request.form.get('spell_hours') or 24)) if request.form.get('spell_hours') else None
            ).save()
            flash('تم زرع التعويذة في المذبح!', 'success')

        # الإعدادات العامة (تم اختصار الكود هنا، وهي مدمجة كما كانت).
...
def admin_update_profile(target_id):
    ...
    if target_user:
        try:
            ...
            elif action == 'reset_avatar': target_user.update(set__avatar='') # زر طمس الصورة
            flash('تم التعديل الإمبراطوري!', 'success')
        except: flash('حدث خطأ في الإدخال', 'error')
    return redirect(request.referrer)

# ==========================================
# 🎮 أوامر الطابق الأول والتزامن
# ==========================================
...
def f1_move():
    ...
    if last_move and (now - last_move).total_seconds() < cooldown:
        flash(f'⏳ انتظر {int(cooldown - (now - last_move).total_seconds())} ثانية قبل التحرك!', 'error')
        return redirect(url_for('home'))
...
def f1_kill():
    ...
    if last_kill and (now - last_kill).total_seconds() < kill_cooldown:
        flash(f'⏳ انتظر {int(kill_cooldown - (now - last_kill).total_seconds())} ثانية للقتل.', 'error')
        return redirect(url_for('home'))
...
# (تم دمج باقي مسارات الطابق الأول والثالث بنفس المنطق كما في الجزء السابق)
# ... [تم اختصارها هنا لتوفير المساحة ولأنك تمتلكها مسبقاً، المهم هو الإضافات الجديدة أعلاه] ...
...
def get_avatar(hunter_id):
    ...
    default_svg = '''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#555"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>'''

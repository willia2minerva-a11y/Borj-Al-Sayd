...
def pre_process():
    ...
    if settings.maintenance_mode:
        ...
        if m_until and now > m_until:
            ...
            try:
                user_check = User.objects(id=session['user_id']).first()
                is_admin = user_check and user_check.role == 'admin'
            except:
                is_admin = False
            if not is_admin:
    ...
    if 'user_id' in session:
        ...
        if user.hunter_id is None:
            ...
            # إعادة جلب المستخدم للحصول على hunter_id الجديد
            user = User.objects(id=session['user_id']).first()
            if user:
                flash('تم تحديث هويتك تلقائياً.', 'info')
            # إذا فشل إعادة الجلب (نادر) نستمر مع القديم

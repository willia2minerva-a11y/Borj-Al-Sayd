...
# حفظ الجلسة لعام كامل لكي لا يطرد اللاعبون عند تحديث السيرفر
...
def wake_up_maze(settings, now):
    """إزاحة الزمن لكي لا يخسر اللاعبون وقتهم ولا يعاقبوا على الخمول أثناء السبات"""
    ...
    # حماية اللاعبين من الموت بالخمول عبر إزاحة آخر نشاط لهم
...
def pre_process():
        ...
        if not settings.sleep_mode_active:
            ...
            if settings.gates_mode_active and settings.gates_end_time and now >= settings.gates_end_time:
                ...
                for hu in hidden_users:
                    ...
                    if real_fate == 'passed': hu.update(set__zone='الطابق 1', set__gate_status='passed')
            ...
            if getattr(settings, 'floor1_mode_active', False):
                # فك الظلام الدامس أو فتح الغرف المغلقة إن انتهى الوقت
                ...
                for gid in User.objects(status='active', group_id__gt=0).distinct('group_id'):
                    ...
                    if total_living > 0:
                        ...
                        # حساب شريط الأحجار (الهدف الفردي × عدد المجموعة الأصلية التي بدأت)
                        ...
                        # أ. فوز الملعون بالذبح
                        if cursed_count > 0 and (cursed_count / total_living) * 100 >= win_percent:
                            living.update(set__zone='الطابق 2')
                            GroupMessage(group_id=gid, sender_name="النظام", message="🔪 سقط الأبرياء... ونجح الملعون! انتقل الأحياء للطابق الثاني، وهلك البقية.", is_system_msg=True).save()
                            
                        # ب. فوز الأبرياء بجمع الأحجار
                            ...
                            living.filter(is_cursed=False).update(set__zone='الطابق 2')
                        ...
                        # ج. فوز الأبرياء بانعدام الملعونين (ماتوا أو طُردوا)
    ...
    if 'user_id' in session:
        ...
        if user.status == 'active' and user.quicksand_lock_until and now < user.quicksand_lock_until: return render_template('locked.html', message=f'مقيّد في الرمال لمدة {(user.quicksand_lock_until - now).seconds // 60} دقائق')
        if user.gate_status == 'testing' and request.endpoint not in ['submit_gate_test', 'logout']: return render_template('gate_test.html', message=getattr(settings, 'gates_test_question', 'ما هو سر المتاهة؟'), user=user)

    g.user = user

# ==========================================
# 🏰 الساحة الرئيسية ورؤية الغرف
# ==========================================
@app.route('/')
def home():
    ...
    if settings and getattr(settings, 'floor1_mode_active', False) and user and user.status == 'active' and user.group_id > 0:
        ...
        if not is_dark:
            ...
            if user.current_room == 'مرصد الظلال': # Admin Map (أرقام فقط)
            ...
            elif user.current_room == 'غرفة المراقبة': # Cameras (أسماء في غرف محددة مجاورة)
                # يرى الكاميرات في منبع الأنفاس، ومستوصف الأرواح، وقاعة النور الأزلي مثلاً
...
def register():
    if request.method == 'POST':
        ...
        # خوارزمية التسجيل الذكية: أعلى ID + 1
...
def use_item(target_id):
    ...
    # استثناء التوتم للميت نفسه (لكي يحيي نفسه)
    ...
    # نظام التوتم العظيم (العودة للحياة)
    if item_type == 'totem':
        if not getattr(settings, 'war_mode', False):
            flash('التوتم لا يتفعل طاقته إلا في الطابق الثاني (الحرب الشاملة)!', 'error')
...
def f1_move():
    ...
    # فحص الاحتجاز (غلق الأبواب من قبل الملعون)
...
def admin_panel():
    ...
    if request.method == 'POST':
        ...
        # إدارة الأوامر الجماعية والفردية داخل الأدمن
        if act == 'hard_delete_single':
            ...
            if settings.sleep_mode_active:
                ...
                GlobalSettings.objects(setting_name='main_config').update_one(set__sleep_mode_active=True, set__sleep_start_time=datetime.utcnow())
            ...
            if not ns: User.objects(status='active', hunter_id__nin=[1000, 1001]).update(set__health=100) # شفاء إن انتهت

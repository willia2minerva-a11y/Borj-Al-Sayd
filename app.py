...
# ==========================================
# 🎮 أوامر الطابق الأول (الحركة، المهام، الاجتماعات)
# ==========================================
...
def f1_move():
    ...
    if getattr(settings, 'floor1_locked_room', '') == new_room:
        flash(f'غرفة {new_room} مغلقة حالياً!', 'error')
        return redirect(url_for('home'))
...
def f1_complete_task(task_index):
    ...
    if task_index >= len(tasks):
        flash('مهمة غير موجودة!', 'error')
        return redirect(url_for('home'))
        
    task = tasks[task_index]
    if task['room'] != user.current_room:
        flash('أنت لست في الغرفة المطلوبة لهذه المهمة!', 'error')
        return redirect(url_for('home'))
        
    if task.get('completed', False):
        flash('أنجزت هذه المهمة مسبقاً!', 'error')
        return redirect(url_for('home'))
    ...
    user.update(
        ...
        set__f1_tasks__S__completed=True
    )
    
    flash(f'✨ أنجزت مهمة "{task["name"]}" وحصلت على {reward} دينار وحجر كريم!', 'success')
...
@app.route('/f1/meeting', methods=['POST'])
@login_required
def f1_meeting():
    user = g.user
    settings = g.settings
    ...
    if gid_str in meetings:
        flash('اجتماع قائم بالفعل في مجموعتك!', 'error')
        return redirect(url_for('home'))
        
    if getattr(user, 'emergency_used', False):
        flash('استخدمت زر الطوارئ مسبقاً!', 'error')
        return redirect(url_for('home'))
        
    end_time = datetime.utcnow() + timedelta(minutes=5)
    meetings[gid_str] = {'end_time': end_time.isoformat(), 'called_by': user.username}
    
    GlobalSettings.objects(setting_name='main_config').update_one(set__f1_active_meetings=meetings)
    user.update(set__emergency_used=True)
    
    GroupMessage(group_id=gid, sender_name="النظام", message=f"🚨 عقد {user.username} اجتماعاً طارئاً! التصويت متاح لـ 5 دقائق.", is_system_msg=True).save()
...
def f1_vote():
    ...
    if gid_str not in meetings:
        flash('لا يوجد اجتماع في مجموعتك!', 'error')
        return redirect(url_for('home'))
    ...
    target = User.objects(username=target_name, group_id=user.group_id, status='active').first()
...
def f1_vent():
    ...
    if getattr(user, 'used_vent', False):
        flash('استخدمت الفتحة السرية مسبقاً!', 'error')
        return redirect(url_for('home'))
        
    # الفتحات موجودة فقط في غرف محددة
    if user.current_room not in ['الممر العظيم', 'قبو التخزين']:
        flash('لا توجد فتحة تهوية في هذه الغرفة!', 'error')
        return redirect(url_for('home'))
        
    # الانتقال العشوائي
    possible_rooms = ['الساحة', 'قاعة المخطوطات', 'مذبح الطلاسم']
    new_room = random.choice(possible_rooms)
    
    user.update(set__current_room=new_room, set__used_vent=True)
    flash(f'تسللت عبر الفتحة السرية وظهرت في {new_room}!', 'success')
    return redirect(url_for('home'))

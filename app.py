...
def check_achievements(user):
    try:
        ...
        if getattr(user, 'stats_ghosts_caught', 0) >= 5 and 'صائد الأشباح 👻' not in user_achs:
            user.achievements.append('صائد الأشباح 👻')
        ...
        if getattr(user, 'stats_puzzles_solved', 0) >= 5 and 'حكيم سيفار 📜' not in user_achs:
            user.achievements.append('حكيم سيفار 📜')
        ...
        if getattr(user, 'stats_items_bought', 0) >= 5 and 'التاجر الخبير 🐪' not in user_achs:
            user.achievements.append('التاجر الخبير 🐪')
            new_ach.append('التاجر الخبير 🐪')
            
        if len(getattr(user, 'friends', []) or []) >= 5 and 'حليف القوم 🤝' not in user_achs:
            user.achievements.append('حليف القوم 🤝')
...
def check_lazy_death_and_bleed(user, settings):
    try:
        if not user or getattr(user, 'role', '') == 'admin' or getattr(user, 'status', '') != 'active': 
            return
        ...
        if (now - last_act).total_seconds() / 3600.0 > 72:
            ...
            user.freeze_reason = 'ابتلعته الرمال لغيابه 72 ساعة'
            ...
            if now > safe_until:
                ...
                if bleed_rate > 0 and minutes_passed >= bleed_rate:
                    ...
                    if user.health <= 0:
                        user.health = 0
                        user.status = 'eliminated'
                        user.freeze_reason = 'نزف حتى الموت في المعركة'
...
def inject_notifications():
    notifs = {
        'un_news': 0, 'un_puz': 0, 'un_dec': 0, 'un_store': 0, 
        'current_user': None, 'war_settings': None, 'battle_logs': []
    }
...
def update_settings():
    ...
    if action == 'change_avatar':
        ...
        if last_change and (now - last_change).days < 15: 
            flash('يسمح بتغيير الاسم كل 15 يوماً فقط!', 'error')
        ...
        if last_pw_change and (now - last_pw_change).days < 1: 
        ...
        elif not check_password_hash(getattr(user, 'password_hash', ''), old_pw): 
            ...
            log_action(f"🔒 قام {user.username} بتغيير كلمة السر", "system")
...
def puzzles():
    ...
    if request.method == 'POST':
        ...
        if puzzle and guess == getattr(puzzle, 'puzzle_answer', '') and str(user.id) not in getattr(puzzle, 'winners_list', []):
    ...
    if request.method == 'POST':
        ...
        try:
            if act == 'moderate_dec':
                ...
                fates = {
                    1: request.form.get('fate_1'), 
                    2: request.form.get('fate_2'), 
                    3: request.form.get('fate_3')
                }
                ...
                if slackers.count() > 0 and active_voters.count() > 0:
                    ...
                    for v in active_voters: 
                        v.survival_votes = getattr(v, 'survival_votes', 0) + bonus
                        v.save()
                for s_user in slackers: 
                    s_user.status = 'eliminated'

...
# استيراد PIL مع معالجة الخطأ (لتجنب مشكلة عدم وجود المكتبة)
try:
    from PIL import Image
    HAS_PIL = True
except ImportError:
    HAS_PIL = False
    print("⚠️ PIL (Pillow) غير مثبت. سيتم تعطيل ضغط الصور.")
    ...
    if action == 'change_avatar':
        ...
        if file and file.filename != '':
            ...
            if len(data) > app.config['MAX_CONTENT_LENGTH']:
                flash('حجم الصورة كبير جداً (يجب أن يكون أقل من 2 ميجا).', 'error')
            else:
...
def altar():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            app.logger.error(f"Spell error: {traceback.format_exc()}")
            flash('حدث خطأ أثناء تنفيذ التعويذة، حاول مرة أخرى.', 'error')
...
def admin_panel():
        ...
        try:
            if act == 'setup_maintenance':
                ...
                if not new_state:
                    User.objects(status='active').update(health=100)
                _settings_cache['timestamp'] = 0
                flash(f'{"تم إشعال الحرب الشاملة!" if new_state else "تم إيقاف الحرب الشاملة."}', 'success')
            elif act == 'toggle_final_battle':
                new_state = not settings.final_battle_mode
                GlobalSettings.objects(setting_name='main_config').update_one(
                    set__final_battle_mode=new_state
                )
                _settings_cache['timestamp'] = 0
                flash(f'{"تم إطلاق المعركة الأخيرة!" if new_state else "تم إنهاء المعركة الأخيرة."}', 'success')
            elif act == 'add_news':
                News(
                ...
                flash('تم إضافة الفخ/الشبح.', 'success')
    ...
    spells = SpellConfig.objects().order_by('-created_at')  # جلب التعويذات

...
# تكوين قاعدة البيانات مع اتصال فوري ومهلات
...
# متغير لتخزين الإعدادات مؤقتاً
_settings_cache = {'data': None, 'timestamp': 0}
_SETTINGS_CACHE_TTL = 30  # ثانية
...
def get_cached_settings():
    ...
    if _settings_cache['data'] is None or (now - _settings_cache['timestamp']) > _SETTINGS_CACHE_TTL:
        try:
            ...
            # في حالة الخطأ، إعادة البيانات القديمة إن وجدت
            if _settings_cache['data'] is None:
                raise e
    return _settings_cache['data']

@app.before_request
def fast_health_check():
    # فحص سريع للحفاظ على استجابة Render
    if request.method == 'HEAD' or request.path == '/health':
        return "OK", 200

@app.before_request
def pre_process():
    # تجاهل المسارات الثابتة وغير الضرورية
    if request.endpoint in ['static', 'get_avatar', 'fast_health_check']:
        return
    
    # تحديث الإعدادات (يتم التخزين المؤقت داخلياً)
    try:
        g.settings = get_cached_settings()
    except Exception as e:
        return f"<div style='direction:rtl; background:#0a0a0a; color:#e74c3c; padding:30px; text-align:center;'><h1>🚨 قاعدة البيانات معطلة!</h1><p>تأكد من رابط MONGO_URI في إعدادات Render.</p><p style='font-family:monospace; direction:ltr;'>{str(e)}</p></div>", 503
    
    # معالجة انتهاء الحرب والبوابات والطابق الثالث (مركزة)
    now = datetime.utcnow()
    settings = g.settings
    
    if settings:
        # إنهاء الحرب إذا انتهى الوقت
        if getattr(settings, 'war_mode', False) and getattr(settings, 'war_end_time', None) and now >= settings.war_end_time:
            ...
            _settings_cache['timestamp'] = 0  # إعادة تحميل الإعدادات
        ...
        if getattr(settings, 'floor3_mode_active', False) and getattr(settings, 'vote_end_time', None) and now >= settings.vote_end_time:
            ...
            if active_voters.count() > 0 and total_dinars > 0:
                ...
                for v in active_voters:
                    v.points = getattr(v, 'points', 0) + bonus
                    v.save()
            for s in slackers:
                s.status = 'eliminated'
    ...
    if 'user_id' in session:
        try:
            # جلب الحقول الأساسية فقط مع only
    ...
    # تخزين المستخدم في g لاستخدامه في السياق
    g.user = user

def check_achievements(user):
    """فحص الإنجازات وإضافتها - تحسين باستخدام update"""
    new_ach = []
...
def check_lazy_death_and_bleed(user, settings):
    """فحص الموت البطيء والنزيف - محسن باستخدام update_one"""
    ...
    if (now - last_act).total_seconds() / 3600.0 > 72:
        User.objects(id=user.id).update(
            ...
            set__freeze_reason='ابتلعته الرمال'
        )
        return
    ...
    if getattr(settings, 'war_mode', False):
        ...
        safe_until = last_action + timedelta(minutes=120)  # 120 دقيقة أمان
        if now > safe_until:
            ...
            if bleed_rate > 0 and minutes_passed >= bleed_rate:
                ...
                if new_health <= 0:
                    User.objects(id=user.id).update(
                        set__health=0,
                        set__status='eliminated',
                        set__freeze_reason='نزف حتى الموت'
                    )
                    GlobalSettings.objects(setting_name='main_config').update_inc(dead_count=1)
                    # التحقق من هدف الحرب
                    updated_settings = GlobalSettings.objects(setting_name='main_config').first()
                    if (updated_settings.dead_count or 0) >= getattr(settings, 'war_kill_target', 15):
                        GlobalSettings.objects(setting_name='main_config').update_one(set__war_mode=False)
                        User.objects(status='active', role='hunter', hunter_id__ne=1000).update(set__zone='الطابق 2')
                else:
                    User.objects(id=user.id).update(
                        set__health=new_health,
                        set__last_health_check=now
                    )
...
def home():
    ...
    return render_template('index.html', alive_count=alive_count, dead_count=dead_count, emperor=emperor)
    ...
    if transfer_type == 'points':
        try:
            ...
            if 0 < amt <= sender.points:
                sender.points -= amt
                receiver.points += amt
                sender.loyalty_points = sender.loyalty_points + 2
                sender.save()
                receiver.save()
        ...
        if itm in sender.inventory:
            ...
            sender.loyalty_points = sender.loyalty_points + 5
            sender.save()
            receiver.save()
...
def use_item(target_id):
    ...
    if item_type == 'weapon' and is_combat_active and target.hunter_id not in attacker.friends:
        ...
        if target.id == attacker.id or target.hunter_id in attacker.friends:
            ...
            if target.id != attacker.id:
                attacker.loyalty_points = attacker.loyalty_points + 5
            target.save()
            attacker.inventory.remove(item_name)
            attacker.last_action_time = now
            attacker.save()
        ...
        if stolen_item in target.inventory:
            if any('حجاب' in i or 'درع' in i or 'عباءة' in i for i in target.inventory):
                ...
                attacker.inventory.remove(item_name)
                attacker.intelligence_points = attacker.intelligence_points + 5
                attacker.save()
                target.save()
...
def add_friend():
    ...
    if target.role in ['ghost', 'cursed_ghost']:
        ...
        if target.role == 'cursed_ghost' and trap:
            ...
            user.stats_ghosts_caught = user.stats_ghosts_caught + 1
            trap.current_winners = trap.current_winners + 1
            trap.winners_list.append(str(user.id))
            user.intelligence_points = user.intelligence_points + 10
            user.save()
            trap.save()
...
def puzzles():
    ...
    if request.method == 'POST':
        ...
        if puzzle and str(guess) == str(puzzle.puzzle_answer) and str(user.id) not in puzzle.winners_list:
            if puzzle.current_winners < puzzle.max_winners:
                user.points += puzzle.reward_points
                user.stats_puzzles_solved = user.stats_puzzles_solved + 1
                puzzle.winners_list.append(str(user.id))
                puzzle.current_winners = puzzle.current_winners + 1
                user.intelligence_points = user.intelligence_points + 10
                user.save()
                puzzle.save()
                check_achievements(user)
        ...
        return redirect(url_for('puzzles'))
    User.objects(id=user.id).update_one(set__last_seen_puzzles=datetime.utcnow())
...
def secret_link(puzzle_id):
    ...
    if puzzle.puzzle_type == 'quicksand_trap':
        ...
        puzzle.current_winners = puzzle.current_winners + 1
        puzzle.winners_list.append(str(user.id))
        user.intelligence_points = user.intelligence_points + 15
        user.save()
        puzzle.save()
...
def declarations():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('declarations'))
    User.objects(id=user.id).update_one(set__last_seen_decs=datetime.utcnow())
    
    approved_decs = list(News.objects(category='declaration', status='approved').order_by('-created_at'))
    # إضافة الحقول التفاعلية
    ...
    if user.points >= item.price:
        if item.is_mirage:
            ...
            if item.is_luck:
                ...
                user.stats_items_bought = user.stats_items_bought + 1
    ...
    try:
        ...
        if len(set(tids)) == 5 and sum(amts) == 100 and 1000 not in tids and user.hunter_id not in tids:
            for i, tid in enumerate(tids):
                ...
                if target_user:
                    target_user.survival_votes = target_user.survival_votes + amts[i]
                    target_user.save()
            user.has_voted = True
            user.save()
...
def admin_panel():
    ...
    if request.method == 'POST':
        ...
        try:
            if act == 'setup_maintenance':
                ...
                GlobalSettings.objects(setting_name='main_config').update_one(
                    set__final_battle_mode=not settings.final_battle_mode
                )
                _settings_cache['timestamp'] = 0
            elif act == 'add_news':
                News(
    ...
    # إحصائيات البوابات باستخدام العد المباشر
    gate_stats = {
